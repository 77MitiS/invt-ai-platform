 ...
 * the remote Invt server, so the auth code never reaches the server.
 *
...
class OpenAIOAuthServiceFlowModeTest {
    ...
    @AfterEach
    void clearOverride() {
        System.clearProperty("invt.oauth.openai.deployment-mode");
    }
    ...
    void publicHosts_resolveToDeviceCode() throws Exception {
        assertEquals(OAuthFlowMode.DEVICE_CODE, invokeResolve("invt-ai-platform.gacgroup.com"));
    ...
    @Test
    @DisplayName("config override invt.oauth.openai.deployment-mode=local forces LOCAL even on remote host")
    void configOverride_forcesLocal() throws Exception {
        System.setProperty("invt.oauth.openai.deployment-mode", "local");
        assertEquals(OAuthFlowMode.LOCAL, invokeResolve("invt-ai-platform.gacgroup.com"));
    }
    ...
    void configOverride_forcesDeviceCode() throws Exception {
        System.setProperty("invt.oauth.openai.deployment-mode", "device_code");
        ...
        System.setProperty("invt.oauth.openai.deployment-mode", "server");
        assertEquals(OAuthFlowMode.DEVICE_CODE, invokeResolve("localhost"));
    }
    ...
    void configOverride_forcesManualPaste() throws Exception {
        System.setProperty("invt.oauth.openai.deployment-mode", "manual_paste");
    ...
    void configOverride_autoFallsThrough() throws Exception {
        System.setProperty("invt.oauth.openai.deployment-mode", "auto");
        assertEquals(OAuthFlowMode.LOCAL, invokeResolve("localhost"));
        assertEquals(OAuthFlowMode.DEVICE_CODE, invokeResolve("api.mate.vip"));

        System.setProperty("invt.oauth.openai.deployment-mode", "garbage");
        assertEquals(OAuthFlowMode.LOCAL, invokeResolve("localhost"));
    }
    ...
    void pastedUrl_emptyRejected() {
        assertThrows(InvtException.class, () -> service.completeFromPastedUrl(null));
        assertThrows(InvtException.class, () -> service.completeFromPastedUrl(""));
        assertThrows(InvtException.class, () -> service.completeFromPastedUrl("   "));
    }
    ...
    void pastedUrl_noQueryRejected() {
        InvtException ex = assertThrows(InvtException.class,
