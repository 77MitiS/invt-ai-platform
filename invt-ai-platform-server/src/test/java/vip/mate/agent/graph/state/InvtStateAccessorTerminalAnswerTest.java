 ...
 * </ul>
 */
class InvtStateAccessorTerminalAnswerTest {
    ...
    void reactPath_returnsFinalAnswer() {
        ...
        assertEquals("ReAct answer", new InvtStateAccessor(s).terminalAnswer());
    }

    @Test
    void planExecuteLongPath_returnsFinalSummary() {
        OverAllState s = mockState("", "Plan summary text", "");
        assertEquals("Plan summary text", new InvtStateAccessor(s).terminalAnswer());
    }

    @Test
    void planExecuteShortPath_returnsDirectAnswer() {
        OverAllState s = mockState("", "", "Direct quick answer");
        assertEquals("Direct quick answer", new InvtStateAccessor(s).terminalAnswer());
    }

    @Test
    void allEmpty_returnsEmptyString() {
        OverAllState s = mockState("", "", "");
        assertEquals("", new InvtStateAccessor(s).terminalAnswer());
    }
