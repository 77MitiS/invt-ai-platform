...
**How Invt is put together, in one page.**

If you're using Invt, read [Introduction](./intro). If you're building on Invt — adding tools, new channels, custom memory providers, new agent graph nodes — read this page.
...
```
invt/
├── invt-ai-platform-server/          # Spring Boot backend (the heart)
│   └── src/main/java/vip/mate/
│       ├── InvtApplication.java
...
├── invt-ai-platform-ui/              # Vue 3 admin console
├── invt-desktop/         # Electron desktop shell
├── invt-ai-platform-webchat/         # Embeddable chat widget
...
**Invt's agent runtime is not a class hierarchy.** There's no `BaseAgent` → `ReActAgent` → `MyCustomAgent` inheritance chain. The runtime is a **StateGraph** (from `spring-ai-alibaba-graph`) composed of nodes and conditional edges, assembled at runtime by `AgentGraphBuilder`.
...
- `agent/graph/state/InvtStateKeys.java` — the keys for the shared state object
- `agent/graph/state/InvtStateAccessor.java` — typed accessor for the state map (don't touch the map directly)
...
**Adding agent behavior** — create a new node in `agent/graph/node/` or a new edge dispatcher in `agent/graph/edge/`. Wire it into `AgentGraphBuilder`. Read and write state through `InvtStateAccessor`.
...
For deeper customization, add a new node in `agent/graph/node/` or a new dispatcher in `agent/graph/edge/`. Wire it into `AgentGraphBuilder` behind a config flag. State access goes through `InvtStateAccessor`.

---

## Persistence — one schema, two databases

Invt uses **MyBatis Plus** (not JPA) for database access. Conventions:
...
Invt uses **Spring MVC**, not Spring WebFlux. WebFlux is explicitly excluded from the dependency graph.
...
2. **Desktop** — Electron shell in `invt-desktop/` bundles JRE 21 and the JAR. Users never install Java. Auto-update via electron-updater.
3. **Docker** — `docker-compose.yml` with MySQL. Production deployment.

The webchat widget in `invt-ai-platform-webchat/` is a fourth path — an embeddable chat UI you can drop into any website. It talks to the same backend APIs.
