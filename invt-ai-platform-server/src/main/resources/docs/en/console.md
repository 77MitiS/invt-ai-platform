...
The admin console is the Vue 3 SPA that ships with every Invt deployment. It runs in your browser (or inside the Electron desktop window), talks to the Spring Boot backend over REST + SSE, and exposes every capability Invt has — chat, agents, knowledge, tools, skills, channels, security, cron jobs, usage analytics — behind one login.
...
Invt uses domain-driven Pinia stores. Each store owns its slice of state exclusively.
...
pnpm build        # vue-tsc + vite build into ../invt-ai-platform-server/.../static
pnpm lint         # ESLint
```
