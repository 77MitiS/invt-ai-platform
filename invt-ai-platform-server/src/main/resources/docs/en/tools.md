...
**1. Built-in tools.** The twenty tools that ship with Invt — registered into the tool table on startup.

**2. MCP servers.** External processes speaking the Model Context Protocol expose tools dynamically. Invt discovers them via `tools/list` and they appear in the registry alongside built-in ones. See [MCP](./mcp).
...
- **Config** — `invt.tools.disclosure.mode`, default `progressive`; set it to `legacy` to restore the old "advertise everything" behavior.
...
| `InvtDocTool` | Read built-in project documentation | — |
...
Reads the built-in Invt project documentation. Lets an agent answer "how does X work in Invt" questions by consulting actual docs rather than guessing.
...
- **Config** — `invt.skill.disclosure.load-skill-tool.enabled`, default true.
...
Tool Guard is how Invt keeps strong tools from doing stupid things. It's **rule-based**, not a flat dangerous-tools list. Each rule says: *for this tool, with these arguments, in this context, do X* — where X is `allow`, `deny`, or `require_approval`.
