...
**Three places to configure Invt: `application.yml`, environment variables, and the database.**
...
|---------|----------|--------------|
| `default` | H2 file at `./data/invt` | No action needed |
...
spring:
  datasource:
    url: jdbc:h2:file:./data/invt;MODE=MYSQL
    username: sa
    password:
...
spring:
  profiles:
    active: mysql
  datasource:
    url: jdbc:mysql://localhost:3306/invt?useSSL=false&serverTimezone=UTC
    username: root
...
| `INVT_CORS_ALLOWED_ORIGINS` | — | CORS allowlist (recommended in production) |
...
Invt uses **Flyway** for schema migrations:
...
| Field | Value |
|-------|-------|
| JDBC URL | `jdbc:h2:file:./data/invt` |
| Username | `sa` |
| Password | *(empty)* |
...
```sql
CREATE DATABASE invt CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
```
