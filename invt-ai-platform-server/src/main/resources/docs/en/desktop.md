...
That's the desktop app in four sentences. No Java to install. No browser to open. No docker compose file. No port to remember. Invt's desktop edition bundles Electron, a JRE 21 runtime, and the packaged Spring Boot server JAR into a single installer. **Your users never know Java is underneath.**
...
JAR_FILE=$(ls -1 target/invt-ai-platform-server-*.jar | grep -v sources | head -n 1)
cp "$JAR_FILE" ../invt-desktop/resources/app.jar
...
Frontend assets can be **hot-updated independently** — a frontend-only fix doesn't require a new installer. See `invt-desktop/scripts/` and `desktop-ui-hot-update.md` for the hot-update build flow.
...
| OS | Path |
|-----|------|
| macOS | `~/Library/Application Support/Invt/data/` |
| Windows | `%APPDATA%/Invt/data/` |
| Linux | `~/.local/share/Invt/data/` |
...
| Setting | Purpose |
|---------|---------|
| `appId` | `vip.mate.invt` — system registration and code signing |
...
- **macOS** — right-click → **Open** to bypass Gatekeeper (first launch). Production: Apple Developer certificate + notarization. See `invt-desktop/CODESIGNING.md`.
...
The WeCom QR-code authorization flow **must open in an in-app popup** (not the system browser) so the `postMessage` callback works. Invt handles this in `setWindowOpenHandler` — `work.weixin.qq.com` domain opens as an in-app popup window.
