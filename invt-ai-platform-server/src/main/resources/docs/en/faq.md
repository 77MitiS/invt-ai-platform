...
Common questions and real answers. If your question isn't here, check the relevant feature page or open a [GitHub issue](https://github.com/matevip/invt/issues).
...
**Java 17 or higher.** Invt uses features introduced in Java 17 (sealed classes, text blocks, records, pattern matching). Verify with `java -version`.
...
- **Ollama** — local GPU inference; Invt auto-detects it on `localhost:11434` at startup
...
**You also don't need to set any API key as an environment variable to start Invt.** All provider configuration is done through the UI at `Settings → Models` after startup.
...
2. Sign up or log in
3. Create an API key
4. In Invt, go to `Settings → Models → DashScope` and paste it
...
Invt implements **sliding-window renewal** — when a token is within 25% of expiry, the server issues a new one in the `X-New-Token` response header. The frontend handles this automatically.
...
Default 10 minutes, then they expire and become `rejected`. Configure with `invt.tool.guard.approval-timeout-seconds`.
...
**H2 (development / desktop):** stop, copy `./data/invt.mv.db`:

```bash
cp ./data/invt.mv.db ./backup/invt-$(date +%Y%m%d).mv.db
```

**MySQL (production):**

```bash
mysqldump -u root -p invt > invt-backup-$(date +%Y%m%d).sql
```

**Docker:**

```bash
docker exec invt-mysql mysqldump -u root -p${MYSQL_ROOT_PASSWORD} invt > backup.sql
```

**Desktop** data lives in the per-user directory:

- macOS: `~/Library/Application Support/Invt/`
- Windows: `%APPDATA%/Invt/`
- Linux: `~/.local/share/Invt/`
...
- macOS: `~/Library/Logs/Invt/`
- Windows: `%APPDATA%/Invt/logs/`
- Linux: `~/.local/share/Invt/logs/`
...
**Auto-updates** via electron-updater. On startup, checks GitHub Releases and prompts you when a new version is available. Manual download also available from [Releases](https://github.com/matevip/invt/releases).
...
```bash
docker compose logs invt-ai-platform-server
docker compose logs invt-mysql
```
...
```bash
docker exec -it invt-mysql mysql -u root -p invt
```
...
2. JDBC URL: `jdbc:h2:file:./data/invt`
3. Username: `sa`
4. Password: (empty)
...
pnpm build
ls ../invt-ai-platform-server/src/main/resources/static/
...
- [GitHub Issues](https://github.com/matevip/invt/issues) — when the docs don't answer your question
