...
The only recommended production deployment outside the desktop app. One `docker compose up -d` brings up three containers: MySQL, SearXNG, and invt-ai-platform-server.
...
| `invt-ai-platform-server` | Built from `invt-ai-platform-server/Dockerfile` | Spring Boot backend + embedded browser | `18080` |
...
- **Upstream ships with only `html` output enabled**, while invt calls `GET /search?q=...&format=json`. The default image responds to JSON requests with an HTML error page, `SearXNGSearchProvider` fails to parse it, returns empty results, and the UI shows "search temporarily unavailable".
...
# 2. Hit it from inside the invt-ai-platform-server container
docker exec invt-ai-platform-server wget -qO- 'http://searxng:8080/search?q=test&format=json' | head -5
# If this fails, compose networking is the problem.

# 3. Ask an agent to search and tail backend logs
docker compose logs -f invt-ai-platform-server | grep "搜索 provider"
# Expect: 搜索 provider 解析: searxng (source=keyless-fallback)
```

### Using an external SearXNG instance

If you're already running SearXNG elsewhere, point invt at it via `.env`:
...
The backend runtime stage (`invt-ai-platform-server/Dockerfile` stage 3) is based on `mcr.microsoft.com/playwright:v1.52.0-noble` (Ubuntu Noble 24.04, glibc) and installs on top of it:
...
1. `CONFIG_CDP` — if `INVT_BROWSER_CDP_URL` is set, attach to that running Chrome
2. `CONFIG_PATH` — if `INVT_BROWSER_CHROME_PATH` or `CHROME_PATH` env is set, use that exe
3. `CONFIG_CHANNEL` — if `INVT_BROWSER_CHANNEL=chrome|msedge`, use the Playwright channel
...
An LLM generating a malicious URL to dump cloud credentials is therefore a closed loop. If you genuinely need to scrape internal infrastructure from a specific host, either disable via `invt.browser.ssrf-check-enabled` or edit the `UrlSafetyChecker` allowlist. **Think twice before doing this in production.**
...
```sh
git clone https://github.com/matevip/invt.git
cd invt
...
| `INVT_CORS_ALLOWED_ORIGINS` | Production allowlist, e.g. `https://invt-ai-platform.gacgroup.com` |

Then bring the stack up:

```sh
docker compose up -d --build   # first build takes 3-10 minutes
docker compose logs -f invt-ai-platform-server
```
...
**Already optimal.** `invt-ai-platform-server/pom.xml` lists repositories in the order `Maven Central → Google CDN → Aliyun`; Central direct is fastest over US/EU backbones.

### China servers

Flip to Aliyun-first. Either edit the `mvn` lines in `invt-ai-platform-server/Dockerfile` to add `-Paliyun-first`, or (easier) expose it as a build arg:
...
```sh
docker compose build --build-arg MAVEN_PROFILE=aliyun-first invt-ai-platform-server
```
...
| `INVT_BROWSER_CDP_URL` | — | Attach to an external Chrome CDP sidecar |
| `INVT_BROWSER_CHROME_PATH` | — | Override the bundled Chromium with a host-installed browser |
| `INVT_BROWSER_CHANNEL` | — | Force a Playwright channel (`chrome`, `msedge`, ...) |
...
**`invt-ai-platform-server` stays unhealthy at startup**
`docker compose logs invt-ai-platform-server` and look for Flyway migration errors. Nine times out of ten, a special character in `DB_PASSWORD` got eaten by the shell — wrap the value in double quotes in `.env`.

**Browser tool reports "Target page closed" or SIGBUS**
`shm_size: 2gb` didn't take effect. Check the actual value with `docker inspect invt-ai-platform-server | grep ShmSize`. Upgrade Docker Engine to 24.0+ if it's still showing 64 MB.
...
```sh
git pull
docker compose build invt-ai-platform-server   # only rebuild the backend
docker compose up -d invt-ai-platform-server
```

The `mysql_data` volume persists across rebuilds. Flyway runs incremental migrations automatically and self-heals checksum changes on restart. **Version is pinned in `invt-ai-platform-server/pom.xml` and the git tag** — prefer pinning to a tag in production, not tracking `dev`.
