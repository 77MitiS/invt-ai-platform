...
Chat is where you actually work. Everything else in Invt — agents, tools, memory, wiki, channels — exists so that what happens inside this box can be good.
...
One of the questions Invt tries to answer with its chat UI is: **should you trust what the AI just told you?** The default answer elsewhere is "look at the answer and guess". Invt tries to do better.
...
Trust is earned by showing the work. Invt shows the work.
...
Drop a **folder** on the desktop app and the agent gets a reference to the folder's absolute path — it can then walk it with the file-reader or shell tool. Drop a folder on the web and Invt recursively expands it and uploads each file individually.
...
::: tip Added in 1.3.0
When the agent's primary model is text-only (e.g. `deepseek-chat`, `kimi-k2`), uploading an image no longer breaks. The runtime auto-routes through a sidecar. See [issue #87](https://github.com/matevip/invt/issues/87).
:::
...
A conversation is a sequence of messages scoped to a single agent and a single user. Invt stores them in two tables:
...
::: tip Added in 1.4.0
The model selector in the chat header now binds a model **to the conversation**, not as a global switch. See [issue #150](https://github.com/matevip/invt/issues/150).
:::
...
::: tip Added in 1.4.0
The conversation sidebar grew from a plain history list into an actionable operations panel. See [issue #144](https://github.com/matevip/invt/issues/144).
:::
...
Every turn, Invt builds the prompt that actually goes to the LLM. Roughly:
...
See `invt-ai-platform-ui/src/composables/chat/useChat.ts` for a full client implementation.
