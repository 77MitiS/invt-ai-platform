...
The [Channels → WeCom](./channels#wecom) section covers wiring up the channel; this document covers what Invt does **after** the channel is up — every non-obvious optimization, every platform corner the adapter handles, and why.
...
The decision lives in [`ChannelMessageRouter.isSameSender`](https://github.com/anthropics/invt/blob/main/invt-ai-platform-server/src/main/java/vip/mate/channel/ChannelMessageRouter.java). Null-defensive: if either senderId is missing, refuse to merge — better to flush twice than to mis-attribute one fragment.
...
```
[@XuZhanFu] @InvtBot I want to query X
[@xuzf] @InvtBot I want to query Y
```
...
When Invt detects `mp.weixin.qq.com` in the link branch, it appends a directive to the model:
...
1. **Detection**: [`hasRepeatingSuffix`](https://github.com/anthropics/invt/blob/main/invt-ai-platform-server/src/main/java/vip/mate/agent/graph/NodeStreamingChatHelper.java) checks if the buffer ends with the same 24-240 character unit repeated 4+ times consecutively → immediately disposes the upstream subscription
...
**Fix**: in the admin panel, **uncheck** all 7 data permissions. The bot becomes available to all authorized members. Invt uses webhooks for messages, doesn't need data permissions.
...
The bot in a WeCom group must be `@`-mentioned to receive a message. Direct messages (1:1) don't need `@`. Platform behavior, no workaround. Invt doesn't broadcast-listen to all group messages (and couldn't if it tried).
...
```bash
grep "wecom-keepalive" logs/invt.log | tail
```
