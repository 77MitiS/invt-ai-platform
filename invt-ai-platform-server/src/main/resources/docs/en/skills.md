...
|------|--------------------|------------------|
| **`builtin`** | Ships with Invt under `skills/` in the classpath | The core team |
...
- **Built-in** — skills that ship with Invt
...
A skill that's installed isn't necessarily a skill that runs — it might need an API key, a CLI tool, a Invt feature flag toggled on.
...
No `~/.zshrc` edit, no invt restart needed.
...
ACP (Agent Client Protocol) is a protocol that lets external agent clients (Claude Code, Codex, other compatible clients) plug into Invt as skills.
