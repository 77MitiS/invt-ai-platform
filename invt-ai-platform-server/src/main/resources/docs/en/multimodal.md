...
Speech, music, images, video — all first-class in Invt, not tacked on.

Most AI products treat multimodal generation as a plugin you bolt on later. Invt ships with it as core infrastructure: **six image providers, four video providers, three TTS backends, three STT backends, and two music providers**, all unified behind a single tool interface so agents can call any of them without knowing which vendor is underneath.
...
Image and video generation often takes longer than a normal agent turn. Invt handles this cleanly:
