...
Invt's LLM Wiki does something different. Drop raw material into a knowledge base and the system reads it, digests it, and writes structured Wiki pages — each with a summary, backlinks, and provenance pointers back to the source passage. You can open any page and read it. You can edit it. Agents read summaries automatically and pull full pages on demand.
...
Invt's LLM Wiki **is the same idea, raised into a product**:
...
- Not an isolated tool — the **knowledge layer of Invt's agent operating system**, threaded through memory, agents, and channel delivery
...
- **Scan a local directory** — desktop only. Point at a folder and Invt walks it recursively, respecting `.gitignore`, importing everything that looks like text.
