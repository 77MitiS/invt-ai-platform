...
Grab the latest installer from [GitHub Releases](https://github.com/matevip/invt/releases).

- **Windows** — `Invt-Setup-x.y.z.exe`
- **macOS** — `Invt-x.y.z.dmg`
- **Linux** — `Invt-x.y.z.AppImage`
...
- **Ollama** — local GPU users; Invt auto-detects `localhost:11434`
...
- **Backend never boots** — Check `~/.invt/logs/app.log` (Windows: `%USERPROFILE%\.invt\logs\`). Nine times out of ten it's a port conflict on 18088.
...
- **Still broken** — Open an issue on [GitHub](https://github.com/matevip/invt/issues) with the tail of `app.log`. We read them.

---

## Other ways to run Invt
...
- **From source** — `mvn spring-boot:run` in `invt-ai-platform-server/` and `pnpm dev` in `invt-ai-platform-ui/`. See [Contributing](./contributing).
