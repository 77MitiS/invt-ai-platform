...
description: Invt's multi-agent system runs in two modes — ReAct for real-time reasoning and Plan-and-Execute for complex task decomposition. Agents can delegate to one another for true multi-agent collaboration.
head:
  - - meta
    - name: keywords
...
This default deny list is tunable via `invt.delegation.child-denied-tools`.
...
Not every question deserves deep reasoning, but some do. Invt lets you turn on deep thinking per agent, per conversation:
