...
Invt doesn't care which LLM you use. It talks to every mainstream provider through five protocol adapters, supports 15+ cloud providers and 4 local runtimes, and lets you swap models at runtime without touching agent configuration. The only opinion Invt has is that you should **start with one and add more when you need them** — not configure everything on day one.
...
::: tip New in 1.4.0
Gemini no longer rides on an OpenAI-compatibility shim — Invt talks to Google's **native `generateContent` API** directly.
:::

Plenty of products bolt Gemini on as "just another OpenAI-compatible endpoint" and then hit walls around system instructions, function calling, and inline images. Invt speaks Gemini's own protocol instead:
...
**A fresh Invt install has an empty provider list. That's deliberate.**
...
Have a ChatGPT Plus or Pro account? Invt can talk to OpenAI's chat endpoint through **browser-based OAuth** — log in the way you normally would, your subscription is used directly. GPT-4o, o3, and o4-mini become available immediately.
...
Browser-callback OAuth needs the IDP's redirect to land back on a `localhost` port that *your* browser can reach. That's fine when Invt runs on your laptop and breaks the moment you put it on a server, in a container, or on a host that doesn't expose a loopback socket to your client.
...
**How Invt decides which flow to use:**

| `invt.oauth.openai.deployment-mode` | Behaviour |
|---|---|
...
- **Browser callback** — local install, browser pops up, you click through, token lands in Invt
...
Providers that expose a model list (OpenAI, Ollama, LM Studio, OpenRouter, etc.) support **Model Discovery** — one click and Invt fetches every model the provider offers.
...
Restart Invt. Auto-discovered, added, enabled.
...
::: tip New in 1.4.0 ([issue #79](https://github.com/matevip/invt/issues/79))
...
**Xiaomi MiMo thinking-mode multi-turn fix** ([issue #189](https://github.com/matevip/invt/issues/189)): MiMo's `reasoning_content` is now kept correctly across turns in thinking mode, instead of being lost on subsequent turns.
...
Invt uses a single **active model** as the global default. Agents that don't specify their own use it.
...
::: tip New in 1.4.0
- **Per-conversation model selection** ([issue #150](https://github.com/matevip/invt/issues/150)): in the chat UI you can switch the model for **just the current conversation**, without touching the global active model or any other conversation. See [Chat & Messaging](./chat).
...
::: tip Added in 1.3.0
Lets a text-only primary model still answer questions about uploaded images. See [issue #87](https://github.com/matevip/invt/issues/87).
:::
...
When does it fire? `MultimodalRouter` ([source](https://github.com/matevip/invt/blob/main/invt-ai-platform-server/src/main/java/vip/mate/llm/routing/MultimodalRouter.java)) decides per turn:
