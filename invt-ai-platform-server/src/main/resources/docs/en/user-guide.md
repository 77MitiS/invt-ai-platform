...
You opened Invt because you want AI to do work for you. Not because you want to learn new software.
...
**A fresh Invt install has an empty provider list. That's deliberate — you don't need to see 16 providers, you need one that works.**
...
Invt's memory doesn't require manual management. After each conversation, the system automatically extracts key information and writes it to memory. Next time, the agent works with that context.
...
|---------|-------------------|
| Backend won't start | Port 18088 is taken. Check `~/.invt/logs/app.log` |
...
| Still broken | [GitHub Issues](https://github.com/matevip/invt/issues) with the tail of `app.log` |
