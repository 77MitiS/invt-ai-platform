...
description: Invt acts as an MCP client, connecting to any external tool server via Model Context Protocol. JSON-RPC dynamic discovery, SSE/stdio dual transport, seamless unification with built-in tools.
head:
  - - meta
    - name: keywords
...
**MCP is how Invt talks to tools someone else built.**

Model Context Protocol is an open standard from Anthropic for connecting AI models to external tools and data. An MCP server is a process — local or remote — that advertises a set of tools over JSON-RPC. Invt acts as an MCP *client*: it connects, discovers tools via `tools/list`, and exposes them to your agents as if they were native. **From the agent's point of view, there's no difference between a built-in `@Tool` Spring bean and a tool coming from an MCP server.**

This is the escape hatch. If you need a capability Invt doesn't ship with — filesystem access for a sandboxed directory, Tavily search, a custom internal data service, a browser automation suite — there's probably already an MCP server for it, and you can plug it in without writing a line of Java.
...
- **MCP Client** — Invt, connecting to servers, discovering tools, forwarding invocations
...
Invt spawns a local child process and exchanges JSON-RPC messages via stdin/stdout.

```
Invt  ── stdin ──►  MCP Server subprocess
          ◄─ stdout ──
```
...
```
Invt  ── HTTP POST ──►  Remote MCP Server
          ◄─ HTTP Stream ──
```
...
| Process management | Invt manages subprocess | External | External |
...
Save. If enabled, Invt auto-attempts to connect and discover tools.
...
1. Confirm the command is in PATH of the user running Invt
...
Subprocesses are cleaned up on normal shutdown. If Invt was force-killed (`kill -9`), subprocesses may remain. `ps aux | grep mcp` and terminate.
