# Invt Desktop UI 热更新设计

## 背景

当前 `invt-desktop` 的运行链路不是“Electron 直接加载 `invt-ai-platform-ui`”，而是：

1. Electron 启动并显示本地 Splash。
2. Electron 用内置 JRE 启动 `invt-ai-platform-server.jar`。
3. `invt-ai-platform-ui` 已提前构建到 `invt-ai-platform-server/src/main/resources/static`。
4. `BrowserWindow` 最终加载 `http://localhost:18088`。

这意味着：

- 现在的 UI 资源和后端 JAR 强绑定。
- 任何 `invt-ai-platform-ui` 改动，都要重新打包 `invt-ai-platform-server.jar`，再跟着桌面安装包一起发布。
- 现有 `electron-updater` 只能做“整包升级”，不能做“仅 UI 升级”。

因此这里要解决的问题，不是开发态 HMR，而是生产态 OTA：让 `invt-ai-platform-ui` 可以脱离桌面安装包独立更新。

## 目标

- 支持 `invt-ai-platform-ui` 独立于 `invt-desktop` 发布。
- UI 更新不要求用户下载新的桌面安装包。
...
`invt-ai-platform-ui` 构建输出目录目前是：

- `../invt-ai-platform-server/src/main/resources/static`

也就是 UI 构建产物直接进入后端静态资源目录，最终随 JAR 发布。

### 2. Desktop 最终加载的是后端地址

`invt-desktop` 主窗口业务页当前加载：
...
1. `invt-ai-platform-ui` 产出独立的静态包（zip）。
2. `invt-desktop` 在启动时检查 UI 更新 Manifest。
...
```text
~/Library/Application Support/Invt/        # macOS 示例
├── data/
├── ui-bundles/
│   ├── current.json
│   ├── 1.0.3+20260405/
...
```json
{
  ...
  "latest": {
    "uiVersion": "1.0.4",
    "buildId": "20260410.1",
    "url": "https://download.example.com/invt/ui/1.0.4/ui-bundle.zip",
    "sha256": "..."
  },
...
需要在 `invt-ai-platform-server` 中新增静态资源优先级：

1. 外部目录 `file:${invt.ui.dir}/`
...
1. `invt-ai-platform-ui` 构建到后端 `static/`
2. Maven 打 JAR
3. Electron 打包安装包
...
1. `invt-ai-platform-ui` 单独构建到临时目录
2. 生成 `meta.json`
3. 打 zip
4. 计算 `sha256`
...
1. 先改 `invt-ai-platform-server`，让它支持“外部目录覆盖 classpath static”。
2. 再改 `invt-desktop`，把 UI 包下载到 `userData/ui-bundles/`，并通过环境变量传给 Java。
3. 再补 Manifest、签名校验和版本展示。
4. 最后再做“运行中更新提示”和“自动回滚”。

## 结论

对当前 Invt 架构，最合适的不是把桌面端改成远程站点壳，而是：
