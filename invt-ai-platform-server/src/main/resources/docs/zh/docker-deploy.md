...
桌面端之外的唯一推荐生产部署方式。一条 `docker compose up -d` 起三个容器：MySQL、SearXNG、invt-ai-platform-server。
...
| `invt-ai-platform-server` | 本地构建 `invt-ai-platform-server/Dockerfile` | Spring Boot 后端 + 内置浏览器 | `18080` |
...
- **上游默认只开 `html` 格式**，invt 后端请求的是 `GET /search?q=...&format=json` —— 默认配置下直接返回 HTML 错误页，`SearXNGSearchProvider` 解析失败返回空列表，UI 显示"搜索暂时不可用"
...
# 2. 从 invt-ai-platform-server 容器内部打
...
后端镜像以 `mcr.microsoft.com/playwright:v1.52.0-noble` 为基础（Ubuntu Noble 24.04，glibc），由 `invt-ai-platform-server/Dockerfile` 的第三阶段拉起，额外装：
...
1. `CONFIG_CDP` —— 配了 `INVT_BROWSER_CDP_URL` → 直接 attach 已运行的 Chrome
2. `CONFIG_PATH` —— 配了 `INVT_BROWSER_CHROME_PATH` 或 `CHROME_PATH` env → 用指定 exe
3. `CONFIG_CHANNEL` —— 配了 `INVT_BROWSER_CHANNEL=chrome|msedge` → 走 Playwright channel
...
`docker-compose.yml` 给 `invt-ai-platform-server` 设了 `shm_size: 2gb`。Docker 默认给每个容器只 64 MB `/dev/shm`，Chromium 用共享内存做 GPU / 页面渲染，跑 3 个 tab 就会 SIGBUS 挂掉，表现为 Playwright `TargetClosedError: Target page, context or browser has been closed`。**不要改小这个值**。
...
也就是说 LLM 生成一个恶意 URL 指向云元数据端点偷凭据这条路是封死的。如果你有内网抓取需求需要放行特定地址，关 `invt.browser.ssrf-check-enabled` 或改 `UrlSafetyChecker` 的白名单。**生产环境谨慎**。
...
| `INVT_CORS_ALLOWED_ORIGINS` | 生产白名单，如 `https://invt-ai-platform.gacgroup.com` |
...
**默认就是最快的**：`invt-ai-platform-server/pom.xml` 里 `<repositories>` 的优先级是 `Maven Central → Google CDN → Aliyun`，Central 直连最快。

### 中国服务器

切 Aliyun 优先：改 `invt-ai-platform-server/Dockerfile` 的 `mvn` 命令加 `-Paliyun-first`，或者（更简单）在 `docker-compose.yml` 加一行 `build args` 传进去。
...
| `INVT_BROWSER_CDP_URL` | — | 接外部 Chrome sidecar（CDP 端点） |
| `INVT_BROWSER_CHROME_PATH` | — | 用宿主机 Chrome 覆盖镜像内置 |
| `INVT_BROWSER_CHANNEL` | — | `chrome` / `msedge` 等，强制指定 Playwright channel |
...
**`invt-ai-platform-server` 启动前就 unhealthy**
`docker compose logs invt-ai-platform-server` 看 Flyway 迁移是否成功。通常是 DB_PASSWORD 含特殊字符被 shell 吃了 —— 用双引号包住。

**浏览器工具报 "Target page closed" / SIGBUS**
`shm_size: 2gb` 没生效。`docker inspect invt-ai-platform-server | grep ShmSize` 看实际值。老版本 Docker Engine 要升级到 24.0+。
...
```sh
git pull
docker compose build invt-ai-platform-server   # 只重建后端
docker compose up -d invt-ai-platform-server
```

MySQL 数据卷（`mysql_data`）不会动，Flyway 自动跑增量迁移 + 自愈 checksum 变化。**版本号写在 `invt-ai-platform-server/pom.xml` 和 git tag**，生产环境建议钉 tag 而不是 `dev` 分支。
