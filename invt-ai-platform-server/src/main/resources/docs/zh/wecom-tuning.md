...
[多渠道接入 → 企业微信](./channels#企业微信) 那一节是把 bot 跑起来；这一篇是把 bot **跑稳**——所有 Invt 在企业微信适配层做过的非显然优化、踩过的平台边角，以及为什么这么处理。
...
代码层面是 [`ChannelMessageRouter.isSameSender`](https://github.com/anthropics/invt/blob/main/invt-ai-platform-server/src/main/java/vip/mate/channel/ChannelMessageRouter.java)。null 防御：任一 senderId 缺失都不合并，宁可多 flush 一次也不要错串归属。
...
Invt 在 link 分支检测到 `mp.weixin.qq.com` 后，会自动给模型追加一段提示：
...
1. **检测**：[`hasRepeatingSuffix`](https://github.com/anthropics/invt/blob/main/invt-ai-platform-server/src/main/java/vip/mate/agent/graph/NodeStreamingChatHelper.java) 探测 buffer 尾部是否被同一个 24~240 字符的 unit 连续重复 4 次以上 → 立即 dispose 上游订阅
...
**解决**：在管理后台**取消勾选**全部 7 项数据权限，bot 即可对所有授权成员可见。Invt 通过 webhook 拿消息，不需要这些数据权限。
...
WeCom 群的 bot 必须被 `@` 才收到消息。私聊不需要 `@`。这是平台行为，没办法绕过。Invt 不会在群里 broadcast 监听所有消息（也做不到）。
