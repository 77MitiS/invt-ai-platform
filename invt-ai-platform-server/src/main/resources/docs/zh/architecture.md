...
**用** Invt 的人看 [项目介绍](./intro)。**在 Invt 上面建东西**的人——加工具、新渠道、自定义记忆 provider、新的 Agent 图节点——看这一页。
...
```
invt/
├── invt-ai-platform-server/          # Spring Boot 后端（心脏）
...
│       ├── skills/           # 捆绑的技能包
│       └── static/           # 前端构建产物
├── invt-ai-platform-ui/              # Vue 3 管理控制台
├── invt-desktop/         # Electron 桌面壳
├── invt-ai-platform-webchat/         # 可嵌入的聊天小部件
...
**Invt 的 Agent 运行时不是一个类层次。** 没有 `BaseAgent` → `ReActAgent` → `MyCustomAgent` 的继承链。运行时是一张**由节点和条件边组成的 StateGraph**（来自 `spring-ai-alibaba-graph`），在运行时由 `AgentGraphBuilder` 装配。
...
- `agent/graph/state/InvtStateKeys.java`——共享 state 对象的 key
- `agent/graph/state/InvtStateAccessor.java`——state map 的类型化访问器（**别直接动 map**）
...
**加 Agent 行为**——在 `agent/graph/node/` 创建新节点，或在 `agent/graph/edge/` 创建新边 dispatcher。把它接进 `AgentGraphBuilder`。通过 `InvtStateAccessor` 读写 state。
...
更深的定制——在 `agent/graph/node/` 加新节点或在 `agent/graph/edge/` 加新 dispatcher。在配置 flag 后面接进 `AgentGraphBuilder`。State 访问走 `InvtStateAccessor`。

---

## 持久化 —— 一份 schema，两种数据库

Invt 用 **MyBatis Plus**（不是 JPA）做数据库访问。约定：
...
Invt 用 **Spring MVC**，不是 Spring WebFlux。**WebFlux 在依赖图里被明确排除。**
...
2. **桌面**——`invt-desktop/` 里的 Electron 壳捆绑 JRE 21 和 JAR。**用户永远不装 Java**。通过 electron-updater 自动更新。
3. **Docker**——`docker-compose.yml` 带 MySQL。生产部署。

`invt-ai-platform-webchat/` 里的 webchat widget 是第四条路——一个你可以丢进任何网站的可嵌入聊天 UI。它和同样的后端 API 对话。
