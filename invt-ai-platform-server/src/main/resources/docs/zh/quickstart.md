...
去 [GitHub Releases](https://github.com/matevip/invt/releases) 拿最新安装包。
...
- **后端起不来**——看 `~/.invt/logs/app.log`（Windows：`%USERPROFILE%\.invt\logs\`）。十有八九是 18088 端口被占了。
- **模型调用报错**——API Key 填错了，或者网络不通。回设置里检查，或者换一家试试。
- **界面白屏**——Ctrl/Cmd + Shift + R 强刷。Electron 的缓存比较顽固。
- **还是不行**——去 [GitHub Issues](https://github.com/matevip/invt/issues) 开一个 Issue，把 `app.log` 的尾巴贴上。我们真的会看。
...
- **从源码跑**——`invt-ai-platform-server/` 里 `mvn spring-boot:run`，`invt-ai-platform-ui/` 里 `pnpm dev`。细节在 [贡献指南](./contributing)。
