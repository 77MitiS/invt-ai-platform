...
Invt 不关心你用哪个 LLM。它通过五个协议适配器跟所有主流供应商对话，支持 15+ 个云端供应商和 4 个本地运行时，你可以在运行时**不动 Agent 配置**直接切模型。Invt 唯一的意见是——**从一个开始，需要再加**，不是第一天就把所有东西配好。
...
::: tip 1.4.0 新增
Gemini 不再走 OpenAI 兼容层——Invt 直接对接 Google 的**原生 `generateContent` API**。
:::

很多产品把 Gemini 当成"又一个 OpenAI 兼容端点"来接，结果在系统指令、函数调用、内联图片这些地方处处碰壁。Invt 走的是 Gemini 自己的协议：
...
有 ChatGPT Plus 或 Pro 账号？Invt 可以通过**浏览器 OAuth** 对接 OpenAI 的 chat 端点——你按平常方式登录，你的订阅被直接使用。GPT-4o、o3、o4-mini 立刻可用。

`设置 → 模型 → 添加供应商 → OpenAI OAuth`。浏览器窗口弹出。Token 交换在后端完成，**凭证不离开你的机器**。

### 设备授权（Device Authorization Grant）—— 远程 / 无头部署专用

浏览器回调式 OAuth 要求 IDP 的重定向能落回 *你的浏览器* 能访问的某个 `localhost` 端口。这事儿在 Invt 跑你笔记本上时没问题，一旦你把它放到服务器、容器、或任何不向客户端暴露 loopback socket 的宿主上，就立刻坏掉。
...
| `invt.oauth.openai.deployment-mode` | 行为 |
|---|---|
...
- **浏览器回调** —— 本地安装，浏览器弹窗，点完授权 token 落到 Invt
...
提供模型列表的供应商（OpenAI、Ollama、LM Studio、OpenRouter 等）支持**模型发现**——一键让 Invt 拉取这个供应商下的所有模型。
...
::: tip 1.4.0 新增（[issue #79](https://github.com/matevip/invt/issues/79)）
...
**小米 MiMo 思考模式多轮修复**（[issue #189](https://github.com/matevip/invt/issues/189)）：MiMo 思考模式的 `reasoning_content` 现在能在多轮对话里正确保留，不再在后续轮次丢失。
...
Invt 用一个**活跃模型**作为全局默认。没有指定自己模型的 Agent 都用它。
...
::: tip 1.4.0 新增
- **按会话选模型**（[issue #150](https://github.com/matevip/invt/issues/150)）：在聊天界面里可以为**当前这一条会话**临时切换模型，不影响全局活跃模型和别的会话。详见 [聊天与消息](./chat)。
...
::: tip 1.3.0 新增
让纯文本主模型也能"看图回答"，参见 [issue #87](https://github.com/matevip/invt/issues/87)。
:::
...
什么时候触发？运行时由 `MultimodalRouter` 决策（[源码](https://github.com/matevip/invt/blob/main/invt-ai-platform-server/src/main/java/vip/mate/llm/routing/MultimodalRouter.java)）：
