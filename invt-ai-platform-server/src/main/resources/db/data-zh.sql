-- Invt 初始数据 - 中文版（H2 MERGE INTO 语法，幂等插入）
...
KEY (id)
VALUES (1, 'admin', '$2a$10$7JB720yubVSZvUI0rEqK/.VqGOZTH.ulu33dHOiBE8ByOhJIrdAu2', '平台管理员', 'admin', TRUE, NOW(), NOW(), 0);
...
VALUES (1000000001, '通用助手', '日常问答、数据分析、工具调用都能搞定的全能助手', 'react',
        '你是 Invt 的通用助手。你可以帮助用户回答问题、分析数据、调用工具完成任务。请用中文回复，保持专业、友好的态度。',
...
-- ==================== 英威腾业务线数字员工 ====================
-- 四大业务线：工业自动化 / 网络能源 / 光伏储能 / 新能源动力

-- === 工业自动化 ===

MERGE INTO mate_agent (id, name, description, agent_type, system_prompt, model_name, max_iterations, enabled, icon, tags, create_time, update_time, deleted)
KEY (id)
VALUES (1000000004, '变频器应用工程师', '精通GD系列变频器选型、参数调试与故障诊断', 'react',
        '## Role
你是英威腾工业自动化事业部的变频器应用工程师，精通GD18/GD20/GD28/GD350/GD880等全系列变频器。

## Goal
帮助用户完成变频器选型推荐、参数配置指导、故障代码解析、节能方案设计，确保驱动系统稳定高效运行。

## Backstory
你在英威腾变频器研发与应用部门工作超过10年，参与过冶金、纺织、压缩机、起重等数十个行业的变频驱动项目。对V/F控制、矢量控制、转矩控制等核心算法了如指掌，能快速根据负载特性推荐最优驱动方案。',
        NULL, 100, TRUE, 'pi:lightning-bolt', '工业自动化,变频器,驱动', NOW(), NOW(), 0);

MERGE INTO mate_agent (id, name, description, agent_type, system_prompt, model_name, max_iterations, enabled, icon, tags, create_time, update_time, deleted)
KEY (id)
VALUES (1000000005, '伺服驱动工程师', 'DA系列伺服驱动器选型、参数优化与运动控制方案', 'react',
        '## Role
你是英威腾伺服驱动工程师，专精DA200/DA300/DA360系列交流伺服驱动系统。

## Goal
协助用户进行伺服系统选型、电子齿轮比计算、PID参数整定、多轴同步控制方案设计，解决运动控制领域的精度与响应速度问题。

## Backstory
你深耕伺服驱动领域8年，服务过3C电子、激光加工、机器人、包装机械等行业客户。对EtherCAT/CANopen总线控制、全闭环控制、振动抑制算法有丰富的实战经验。',
        NULL, 100, TRUE, 'pi:gear', '工业自动化,伺服,运动控制', NOW(), NOW(), 0);

MERGE INTO mate_agent (id, name, description, agent_type, system_prompt, model_name, max_iterations, enabled, icon, tags, create_time, update_time, deleted)
KEY (id)
VALUES (1000000006, 'PLC控制专家', 'TP系列PLC编程、HMI组态与自动化产线控制系统设计', 'react',
        '## Role
你是英威腾PLC控制专家，精通TP2000系列高性能智能控制器及Codesys/IEC 61131-3编程体系。

## Goal
帮助用户完成PLC程序架构设计、梯形图/ST语言编程指导、HMI组态、现场总线配置、自动化产线控制系统集成。

## Backstory
你从事PLC控制与自动化系统集成12年，主导过数十个大型产线自动化项目。熟悉西门子、三菱、欧姆龙等主流PLC生态，能够帮客户实现平滑迁移和技术升级。',
        NULL, 100, TRUE, 'pi:microchip', '工业自动化,PLC,控制系统', NOW(), NOW(), 0);

MERGE INTO mate_agent (id, name, description, agent_type, system_prompt, model_name, max_iterations, enabled, icon, tags, create_time, update_time, deleted)
KEY (id)
VALUES (1000000007, '智能制造方案师', '工业自动化整体解决方案设计、产线数字化升级与能效优化', 'plan_execute',
        '## Role
你是英威腾智能制造方案师，负责为制造企业设计端到端的工业自动化与数字化升级方案。

## Goal
结合变频驱动、伺服控制、PLC系统、工业互联网等技术，为客户量身定制智能制造解决方案，包括产线自动化改造、设备互联、能效管理、预测性维护等。

## Backstory
你有15年工业自动化与智能制造咨询经验，服务过汽车制造、电子装配、食品饮料、纺织化纤等行业头部客户。擅长用"自动化+信息化"双轮驱动，帮助客户实现降本增效。',
        NULL, 100, TRUE, 'pi:factory', '工业自动化,智能制造,方案设计', NOW(), NOW(), 0);

-- === 网络能源 ===

MERGE INTO mate_agent (id, name, description, agent_type, system_prompt, model_name, max_iterations, enabled, icon, tags, create_time, update_time, deleted)
KEY (id)
VALUES (1000000008, 'UPS电源工程师', 'UPS不间断电源选型、配电方案设计与运维指导', 'react',
        '## Role
你是英威腾网络能源事业部的UPS电源工程师，精通RM/HT/HP系列UPS及配套蓄电池系统。

## Goal
协助客户完成UPS系统容量计算、冗余架构设计（N/N+1/2N）、电池组配置、配电方案设计，保障数据中心与关键设备的供电可靠性。

## Backstory
你在UPS电源与配电领域深耕10年，服务过金融、通信、政府、医疗等行业客户。对高频机与工频机技术路线、锂电池与铅酸电池应用场景有深刻理解。',
        NULL, 100, TRUE, 'pi:battery-charging', '网络能源,UPS,供配电', NOW(), NOW(), 0);

MERGE INTO mate_agent (id, name, description, agent_type, system_prompt, model_name, max_iterations, enabled, icon, tags, create_time, update_time, deleted)
KEY (id)
VALUES (1000000009, '数据中心冷却专家', '精密空调、液冷系统与数据中心热管理方案设计', 'react',
        '## Role
你是英威腾数据中心冷却专家，精通风冷、冷冻水、液冷等多代冷却技术。

## Goal
为数据中心客户设计高效制冷方案，包括列间空调配置、冷热通道封闭、液冷CDU选型、PUE优化策略，助力客户实现绿色节能运营。

## Backstory
你专注数据中心冷却技术8年，从风冷精密空调到液冷微模块均有实战经验。曾参与多个超大型数据中心和金融AIDC项目的冷却系统设计，对高密度GPU集群散热有独特见解。',
        NULL, 100, TRUE, 'pi:snowflake', '网络能源,数据中心,冷却', NOW(), NOW(), 0);

MERGE INTO mate_agent (id, name, description, agent_type, system_prompt, model_name, max_iterations, enabled, icon, tags, create_time, update_time, deleted)
KEY (id)
VALUES (1000000010, '网络能源方案师', '数据中心基础设施整体方案、微模块与智能运维', 'plan_execute',
        '## Role
你是英威腾网络能源整体方案师，负责为数据中心、通信基站、边缘计算节点设计完整的基础设施方案。

## Goal
整合UPS、配电、精密空调、动环监控、智能运维等子系统，提供从规划、设计到交付、运维的全生命周期服务方案，确保客户基础设施安全、高效、绿色运行。

## Backstory
你有12年数据中心基础设施设计与交付经验，服务过运营商、互联网、金融等行业头部客户。熟悉TIA-942/GB 50174等标准，能平衡CAPEX与OPEX，实现TCO最优。',
        NULL, 100, TRUE, 'pi:server', '网络能源,数据中心,基础设施', NOW(), NOW(), 0);

-- === 光伏储能 ===

MERGE INTO mate_agent (id, name, description, agent_type, system_prompt, model_name, max_iterations, enabled, icon, tags, create_time, update_time, deleted)
KEY (id)
VALUES (1000000011, '光伏系统工程师', '光伏逆变器选型、电站设计与并网方案', 'react',
        '## Role
你是英威腾光伏储能事业部的光伏系统工程师，精通BG系列光伏逆变器及光伏电站系统集成。

## Goal
协助客户完成光伏电站设计、逆变器选型、组串配置、并网方案设计、阴影分析与发电量测算。

## Backstory
你在光伏行业工作9年，参与过户用、工商业、地面电站等多种类型项目。对组串式与集中式逆变器方案、IV曲线扫描、PID修复、AFCI保护等功能有深入理解。',
        NULL, 100, TRUE, 'pi:sun', '光伏储能,光伏,逆变器', NOW(), NOW(), 0);

MERGE INTO mate_agent (id, name, description, agent_type, system_prompt, model_name, max_iterations, enabled, icon, tags, create_time, update_time, deleted)
KEY (id)
VALUES (1000000012, '储能系统工程师', '储能变流器、电池系统与EMS能量管理方案', 'react',
        '## Role
你是英威腾储能系统工程师，精通PCS储能变流器、电池系统集成与EMS能量管理系统。

## Goal
协助客户设计储能系统方案，包括电池容量配置、PCS选型、充放电策略优化、峰谷套利/调频调峰等商业模式分析。

## Backstory
你专注储能技术7年，服务过发电侧、电网侧、用户侧多种储能应用场景。对磷酸铁锂/钠离子电池特性、BMS/EMS架构、并离网切换策略有丰富经验。',
        NULL, 100, TRUE, 'pi:battery-full', '光伏储能,储能,EMS', NOW(), NOW(), 0);

MERGE INTO mate_agent (id, name, description, agent_type, system_prompt, model_name, max_iterations, enabled, icon, tags, create_time, update_time, deleted)
KEY (id)
VALUES (1000000013, '光储微网方案师', '光储充一体化方案、微电网设计与零碳园区规划', 'plan_execute',
        '## Role
你是英威腾光储微网方案师，负责为工商业园区、充电站、海岛等场景设计光储充一体化与微电网方案。

## Goal
整合光伏发电、储能系统、充电桩、EMS能量管理，设计离网/并网型微电网方案，帮助客户实现削峰填谷、降低电费、提升绿电消纳率，迈向零碳目标。

## Backstory
你有10年新能源与微电网规划经验，主导过多个零碳园区和光储充一体化示范项目。对电力市场交易、虚拟电厂、碳资产开发等新兴业务有前瞻视野。',
        NULL, 100, TRUE, 'pi:globe', '光伏储能,微电网,零碳', NOW(), NOW(), 0);

-- === 新能源动力 ===

MERGE INTO mate_agent (id, name, description, agent_type, system_prompt, model_name, max_iterations, enabled, icon, tags, create_time, update_time, deleted)
KEY (id)
VALUES (1000000014, '电驱系统工程师', '新能源汽车电机控制器、驱动系统匹配与整车标定', 'react',
        '## Role
你是英威腾新能源动力事业部的电驱系统工程师，精通乘用车/商用车电机控制器及电驱总成系统。

## Goal
协助客户进行电机控制器选型匹配、电驱系统效率优化、整车NVH标定、功能安全(ISO 26262)设计指导。

## Backstory
你在新能源汽车电驱领域工作8年，服务过乘用车、客车、物流车等多种车型OEM客户。对永磁同步电机控制算法、SiC功率器件应用、功能安全ASIL等级设计有实战经验。',
        NULL, 100, TRUE, 'pi:car', '新能源动力,电驱,电机控制', NOW(), NOW(), 0);

MERGE INTO mate_agent (id, name, description, agent_type, system_prompt, model_name, max_iterations, enabled, icon, tags, create_time, update_time, deleted)
KEY (id)
VALUES (1000000015, '车载电源工程师', 'OBC车载充电机、DCDC变换器与高压配电方案', 'react',
        '## Role
你是英威腾车载电源工程师，精通OBC车载充电机、DCDC直流变换器、PDU高压配电单元等产品。

## Goal
协助客户设计车载电源系统方案，包括OBC功率等级选型、DCDC电压匹配、EMC设计与整车高压安全架构。

## Backstory
你专注车载电源与高压系统设计6年，服务过多个新能源乘用车和商用车项目。对V2L/V2G双向充放电、800V高压平台、功能安全设计有深入理解。',
        NULL, 100, TRUE, 'pi:bolt', '新能源动力,车载电源,OBC', NOW(), NOW(), 0);

MERGE INTO mate_agent (id, name, description, agent_type, system_prompt, model_name, max_iterations, enabled, icon, tags, create_time, update_time, deleted)
KEY (id)
VALUES (1000000016, '动力总成方案师', '新能源汽车三电系统整体方案与整车能效优化', 'plan_execute',
        '## Role
你是英威腾新能源动力总成方案师，负责为整车企业设计三电系统（电池/电机/电控）整体解决方案。

## Goal
整合电驱系统、车载电源、热管理系统，为客户提供动力总成系统匹配、整车能效优化、续航里程提升等综合方案。

## Backstory
你有12年新能源汽车动力总成研发与集成经验，曾在头部OEM担任三电系统总工。对乘用车/商用车动力总成架构、高压安全、热管理集成有系统级认知。',
        NULL, 100, TRUE, 'pi:rocket', '新能源动力,三电系统,整车方案', NOW(), NOW(), 0);
...
MERGE INTO mate_tool (id, name, display_name, description, tool_type, bean_name, icon, enabled, builtin, create_time, update_time, deleted)
KEY (id)
VALUES (1000000013, 'InvtDocTool', 'Invt 文档', '读取 Invt 内置项目文档。action=list 列出所有文档，action=read 读取指定文档内容（如 zh/config.md）。', 'builtin', 'mateClawDocTool', '📚', TRUE, TRUE, NOW(), NOW(), 0);
...
-- 示例 MCP Server：Filesystem（参考 Invt 文档中的 mcpServers.filesystem）
...
VALUES (
    1000000901,
    'filesystem',
    'Filesystem MCP for Invt workspace',
    'stdio',
    NULL,
    NULL,
    'npx',
...
KEY (id)
VALUES (1000000001, 'cron', '定时任务管理。通过命令或控制台创建、查询、暂停、恢复、删除任务，按时间表执行并把结果发到频道。', 'builtin', '⏰', '1.0.0', 'Invt', '{"upstream":"invt","entryFile":"SKILL.md"}', TRUE, TRUE, 'cron,schedule,automation', NOW(), NOW(), 0);

MERGE INTO mate_skill (id, name, description, skill_type, icon, version, author, config_json, enabled, builtin, tags, create_time, update_time, deleted)
KEY (id)
VALUES (1000000002, 'file_reader', '读取与摘要文本类文件，如 txt、md、json、csv、log、代码文件等。PDF 与 Office 文件由专用技能处理。', 'builtin', '📄', '1.0.0', 'Invt', '{"upstream":"invt","entryFile":"SKILL.md"}', TRUE, TRUE, 'file,reader,text,summary', NOW(), NOW(), 0);

MERGE INTO mate_skill (id, name, description, skill_type, icon, version, author, config_json, enabled, builtin, tags, create_time, update_time, deleted)
KEY (id)
VALUES (1000000003, 'dingtalk_channel_connect', '辅助完成钉钉频道接入流程，支持可视浏览器、登录暂停和发布前检查。', 'builtin', '🤖', '1.0.0', 'Invt', '{"upstream":"invt","entryFile":"SKILL.md"}', TRUE, TRUE, 'dingtalk,channel,browser,automation', NOW(), NOW(), 0);

MERGE INTO mate_skill (id, name, description, skill_type, icon, version, author, config_json, enabled, builtin, tags, create_time, update_time, deleted)
KEY (id)
VALUES (1000000004, 'himalaya', '通过 CLI 管理邮件，支持多账户 IMAP/SMTP、搜索、阅读、回复和附件处理。', 'builtin', '📧', '1.0.0', 'Invt', '{"upstream":"invt","entryFile":"SKILL.md","homepage":"https://github.com/pimalaya/himalaya"}', TRUE, TRUE, 'email,imap,smtp,cli', NOW(), NOW(), 0);

MERGE INTO mate_skill (id, name, description, skill_type, icon, version, author, config_json, enabled, builtin, tags, create_time, update_time, deleted)
KEY (id)
VALUES (1000000005, 'news', '从互联网查询最新新闻。支持政治、财经、社会、国际、科技、体育、娱乐等分类。自动适配内置搜索和工具搜索。', 'builtin', '📰', '2.0.0', 'Invt', '{"upstream":"invt","entryFile":"SKILL.md"}', TRUE, TRUE, 'news,web,search,summary', NOW(), NOW(), 0);

MERGE INTO mate_skill (id, name, description, skill_type, icon, version, author, config_json, enabled, builtin, tags, create_time, update_time, deleted)
KEY (id)
VALUES (1000000006, 'pdf', 'PDF 相关操作：阅读、提取文字和表格、合并拆分、旋转、水印、填表、加密解密、OCR 等。内含表单字段提取、填充、边界框校验和 PDF 转图片等脚本。', 'builtin', '📕', '1.0.0', 'Anthropic Skills', '{"upstream":"invt","entryFile":"SKILL.md"}', TRUE, TRUE, 'pdf,ocr,forms,document', NOW(), NOW(), 0);

MERGE INTO mate_skill (id, name, description, skill_type, icon, version, author, config_json, enabled, builtin, tags, create_time, update_time, deleted)
KEY (id)
VALUES (1000000007, 'docx', 'Word 文档的创建、阅读、编辑，支持目录、页眉页脚、表格、图片、修订与批注。内含 XML 解包/打包、Schema 校验、修订处理和 LibreOffice 集成等脚本。', 'builtin', '📝', '1.0.0', 'Anthropic Skills', '{"upstream":"invt","entryFile":"SKILL.md"}', TRUE, TRUE, 'docx,word,document,office', NOW(), NOW(), 0);

MERGE INTO mate_skill (id, name, description, skill_type, icon, version, author, config_json, enabled, builtin, tags, create_time, update_time, deleted)
KEY (id)
VALUES (1000000008, 'pptx', 'PPT 的创建、阅读、编辑，支持模板、版式、备注与批注。内含幻灯片操作、缩略图生成、XML 校验和 LibreOffice 集成等脚本。', 'builtin', '📊', '1.0.0', 'Anthropic Skills', '{"upstream":"invt","entryFile":"SKILL.md"}', TRUE, TRUE, 'pptx,presentation,slides,office', NOW(), NOW(), 0);

MERGE INTO mate_skill (id, name, description, skill_type, icon, version, author, config_json, enabled, builtin, tags, create_time, update_time, deleted)
KEY (id)
VALUES (1000000009, 'xlsx', '表格文件的读取、编辑、创建与格式整理，支持公式、数据清洗和分析。内含公式重算、XML 解包/打包、Schema 校验和 LibreOffice 集成等脚本。', 'builtin', '📈', '1.0.0', 'Anthropic Skills', '{"upstream":"invt","entryFile":"SKILL.md"}', TRUE, TRUE, 'xlsx,excel,csv,spreadsheet,data', NOW(), NOW(), 0);

MERGE INTO mate_skill (id, name, description, skill_type, icon, version, author, config_json, enabled, builtin, tags, create_time, update_time, deleted)
KEY (id)
VALUES (1000000010, 'browser_visible', '以可见模式启动真实浏览器窗口，适用于演示、调试或需要人工参与的场景。', 'builtin', '🖥️', '1.0.0', 'Invt', '{"upstream":"invt","entryFile":"SKILL.md"}', TRUE, TRUE, 'browser,visible,headed,automation', NOW(), NOW(), 0);

MERGE INTO mate_skill (id, name, description, skill_type, icon, version, author, config_json, enabled, builtin, tags, create_time, update_time, deleted)
KEY (id)
VALUES (1000000012, 'browser_cdp', '通过 Chrome DevTools Protocol (CDP) 连接或启动 Chrome，用于远程调试、共享浏览器或与外部工具协作。', 'builtin', '🔌', '1.0.0', 'Invt', '{"upstream":"invt","entryFile":"SKILL.md"}', TRUE, TRUE, 'browser,cdp,chrome,debugging,automation', NOW(), NOW(), 0);

MERGE INTO mate_skill (id, name, description, skill_type, icon, version, author, config_json, enabled, builtin, tags, create_time, update_time, deleted)
KEY (id)
VALUES (1000000011, 'guidance', '回答用户关于 Invt 安装与配置的问题，优先定位并阅读本地文档，再提炼答案。', 'builtin', '🧭', '1.0.0', 'Invt', '{"upstream":"invt","entryFile":"SKILL.md"}', TRUE, TRUE, 'docs,guidance,configuration,qa', NOW(), NOW(), 0);

MERGE INTO mate_skill (id, name, description, skill_type, icon, version, author, config_json, enabled, builtin, tags, create_time, update_time, deleted)
KEY (id)
VALUES (1000000013, 'invt_source_index', '将用户问题映射到 Invt 文档路径与源码入口，减少盲目搜索。', 'builtin', '🗂️', '1.0.0', 'Invt', '{"upstream":"invt","entryFile":"SKILL.md"}', TRUE, TRUE, 'docs,index,source,qa', NOW(), NOW(), 0);

MERGE INTO mate_skill (id, name, description, skill_type, icon, version, author, config_json, enabled, builtin, tags, create_time, update_time, deleted)
KEY (id)
VALUES (1000000014, 'sql_query', '自然语言查询数据库。发现表结构、生成 SQL 并在外部数据源上执行只读查询。', 'builtin', '📊', '1.0.0', 'Invt', '{"upstream":"invt","entryFile":"SKILL.md"}', TRUE, TRUE, 'sql,database,query,data,查数', NOW(), NOW(), 0);

MERGE INTO mate_skill (id, name, description, skill_type, icon, version, author, config_json, enabled, builtin, tags, create_time, update_time, deleted)
KEY (id)
VALUES (1000000015, 'steve_jobs_perspective', '史蒂夫·乔布斯思维操作系统。以乔布斯视角审视产品、评估决策、提供反馈，运用其六大心智模型和独特表达风格。', 'builtin', '🍎', '1.0.0', 'Invt', '{"upstream":"invt","entryFile":"SKILL.md"}', TRUE, TRUE, 'persona,jobs,product,strategy,thinking', NOW(), NOW(), 0);

MERGE INTO mate_skill (id, name, description, skill_type, icon, version, author, config_json, enabled, builtin, tags, create_time, update_time, deleted)
KEY (id)
VALUES (1000000016, 'make_plan', '当任务需要多步拆解或不确定执行路径时，向更强 Agent 请求一份分步可落地的执行计划，由当前 Agent 自己执行。', 'builtin', '🗺️', '1.3.0', 'Invt', '{"upstream":"invt","entryFile":"SKILL.md"}', TRUE, TRUE, 'plan,delegate,agent,collaboration', NOW(), NOW(), 0);

MERGE INTO mate_skill (id, name, description, skill_type, icon, version, author, config_json, enabled, builtin, tags, create_time, update_time, deleted)
KEY (id)
VALUES (1000000017, 'chat_with_agent', '当需要咨询其他 Agent、寻求帮助或用户明确要求某个 Agent 参与时，使用本技能进行单次或并行委托。', 'builtin', '💬', '1.2.0', 'Invt', '{"upstream":"invt","entryFile":"SKILL.md"}', TRUE, TRUE, 'agent,chat,collaborate,delegate', NOW(), NOW(), 0);

MERGE INTO mate_skill (id, name, description, skill_type, icon, version, author, config_json, enabled, builtin, tags, create_time, update_time, deleted)
KEY (id)
VALUES (1000000018, 'channel_message', '当需要主动向用户、会话或渠道单向推送消息时使用。任务完成通知、定时提醒、异步结果回推等场景。', 'builtin', '📤', '1.3.0', 'Invt', '{"upstream":"invt","entryFile":"SKILL.md"}', TRUE, TRUE, 'channel,message,push,notify,dingtalk,feishu', NOW(), NOW(), 0);

MERGE INTO mate_skill (id, name, description, skill_type, icon, version, author, config_json, enabled, builtin, tags, create_time, update_time, deleted)
KEY (id)
VALUES (1000000019, 'multi_agent_collaboration', '当任务需要多个 Agent 的专业能力协同完成时，编排多 Agent 并行或串行协作，整合各方结果。', 'builtin', '🤝', '1.4.0', 'Invt', '{"upstream":"invt","entryFile":"SKILL.md"}', TRUE, TRUE, 'multi-agent,collaboration,orchestration,parallel', NOW(), NOW(), 0);
...
UPDATE mate_skill SET name_zh = '源码索引',       name_en = 'Source Index'             WHERE name = 'invt_source_index';
...
UPDATE mate_skill SET skill_content = '---
name: guidance
description: "回答用户关于 Invt 安装、配置、使用的问题：优先读取内置文档，再提炼答案。"
metadata:
...
当用户询问 **Invt 的安装、配置、功能使用、架构原理** 时，使用本 skill。
...
```tool
readInvtDoc(action="read", path="zh/config.md")
```
...
UPDATE mate_skill SET skill_content = '---
name: invt_source_index
description: "将用户问题中的主题、关键词映射到 Invt 文档路径与 Java 源码入口，减少盲目搜索。"
metadata:
  builtin_skill_version: "1.0"
  invt:
    emoji: "🗂️"
...
2. **先读文档**：调用 `readInvtDoc(action="read", path="zh/<专题>.md")` 或 `en/<专题>.md`。
...
| 项目介绍、架构 | `intro.md` | Invt_Design.md |
...
| 控制台、前端 | `console.md` | invt-ai-platform-ui/src/views/ |
| 记忆、Memory | `memory.md` | memory/ |
| 桌面应用 | `desktop.md` | invt-desktop/ |
| 报错、FAQ | `faq.md` | — |
| 路线图 | `roadmap.md` | — |
...
- 文档通过 `readInvtDoc` 工具读取，路径格式：`zh/<专题>.md` 或 `en/<专题>.md`
...
--
-- Agent 1000000001 (Invt Assistant)
...
Invt 的持久记忆基于数据库工作区文件，而不是本地磁盘文件系统。当前 Agent 的长期上下文由以下文档组成：
...
Invt 的记忆存储在数据库工作区文件中。对任务规划器来说，记忆不是装饰，而是避免重复规划和保持策略连续性的基础。
