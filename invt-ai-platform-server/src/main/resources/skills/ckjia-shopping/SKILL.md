...
- API key 由管理员在 ckjia 控制台申请后填入 invt `Settings ▸ MCP Connections` 的 `headers_json`，使用 `${CKJIA_MCP_KEY}` 环境变量占位符避免明文落库
...
4. 在 invt 部署环境配 `CKJIA_MCP_KEY=ckjia_mcp_live_xxx`
