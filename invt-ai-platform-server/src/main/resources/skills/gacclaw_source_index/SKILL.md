...
version: "1.0.0"
description: "将用户问题映射到 INVT AI Platform 文档路径与源码入口，减少盲目搜索。回答'XX 功能在哪里实现'、'怎么修改 YY 逻辑'等源码定位问题。"
dependencies:
  tools:
    - readInvtDoc
    - read_file
...
```
readInvtDoc(action="read", path="zh/architecture.md")
```
...
- 路径均相对于 `invt-ai-platform-server/src/main/java/vip/mate/`（Java 文件）或 `invt-ai-platform-server/src/main/resources/`（资源文件）
- 如找不到精确文件，先用 `readInvtDoc` 搜索，再用 `read_file` 读取 CLAUDE.md 获取最新架构描述
