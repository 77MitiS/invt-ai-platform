...
version: "1.3.0"
optional: true
description: "使用可见浏览器自动完成钉钉渠道接入配置。遇到登录页必须暂停等待用户手动登录后继续。"
dependencies:
  tools:
    - browser_use
...
2. 再用文件上传操作（Invt browser_use 支持 file input 的 `type` 操作传入路径）
...
**方式 A — 控制台前端：**
> 进入 Invt 管理界面 → 渠道 → 新建渠道 → 选择钉钉 → 填入 Client ID 和 Client Secret
