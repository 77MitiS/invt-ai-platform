...
public class GitSkillFetcher {
     ...
     * Resolution order: {@code invt.skill.github-token} property → {@code GITHUB_TOKEN}
    ...
    public GitSkillFetcher(
            ...
            @Value("${invt.skill.github-token:}") String configuredGithubToken) {
    ...
    public SkillBundle fetch(String repoUrl, String ref, String subPath) {
        Path tempDir = null;
        try {
            tempDir = Files.createTempDirectory("invt-skill-install-");
