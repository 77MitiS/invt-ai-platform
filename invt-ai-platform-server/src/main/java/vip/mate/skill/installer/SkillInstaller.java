 ...
 * <p>
 * 支持异步安装（task_id 轮询模式），参考 Invt 实现。
 *
 * @author Invt Team
 */
@Slf4j
...
public class SkillInstaller {
    ...
    public Map<String, Object> installFromBundle(SkillBundle bundle, boolean enable, boolean overwrite,
        ...
        if (skillName == null || skillName.isBlank()) {
            throw new vip.mate.exception.InvtException("err.skill.name_required", "Cannot determine skill name from bundle");
        }

        boolean exists = skillService.listSkills().stream()
                .anyMatch(s -> s.getName().equals(skillName));
        if (exists && !overwrite) {
            throw new vip.mate.exception.InvtException("err.skill.name_exists",
