...
public class SkillLifecycleService {
    ...
    public SkillEntity restore(Long id) {
        ...
        if (skill == null) {
            throw new InvtException("err.skill.not_found", 404, "Skill not found: " + id);
        }
        if (!"archived".equals(skill.getLifecycleState())) {
            throw new InvtException("err.skill.not_archived", 409,
        ...
        switch (fs) {
            ...
            case MISSING -> {
                if (skill.getSkillContent() == null || skill.getSkillContent().isBlank()) {
                    throw new InvtException("err.skill.unrecoverable", 409,
            ...
            case FAILED -> throw new InvtException("err.skill.restore_failed", 500,
