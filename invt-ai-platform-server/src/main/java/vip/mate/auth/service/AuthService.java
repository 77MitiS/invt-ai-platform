...
public class AuthService {
    ...
    @Value("${invt.jwt.secret:Invt-Secret-Key-2024-Very-Long-String}")
    private String jwtSecret;

    @Value("${invt.jwt.expiration:86400000}")
    private long jwtExpiration;

    @Value("${invt.jwt.renewal-threshold:7200000}")
    ...
    public LoginResponse login(LoginRequest request) {
        ...
        if (user == null || !passwordEncoder.matches(request.getPassword(), user.getPassword())) {
            throw new InvtException("err.auth.invalid_credentials", 401, "用户名或密码错误");
        }
    ...
    public UserEntity createUser(UserEntity user) {
        ...
        if (count > 0) {
            throw new InvtException("err.auth.username_exists", "用户名已存在: " + user.getUsername());
        }
        if (user.getPassword() == null || user.getPassword().isBlank()) {
            throw new InvtException("err.auth.password_required", "Password is required");
        }
    ...
    public void resetPassword(Long userId, String newPassword) {
        ...
        if (user == null) {
            throw new InvtException("err.auth.user_not_found", "用户不存在");
        }
     ...
     * the same {@link InvtException} keys as {@link #changePassword} so
     * the user-facing error message stays consistent.
     *
     * @throws InvtException {@code err.auth.user_not_found} when the user
    ...
    public void verifyCurrentUserPassword(Long userId, String rawPassword) {
        ...
        if (rawPassword == null || !passwordEncoder.matches(rawPassword, user.getPassword())) {
            ...
            throw new InvtException("err.auth.wrong_password", 403, "原密码错误");
        }
    }
