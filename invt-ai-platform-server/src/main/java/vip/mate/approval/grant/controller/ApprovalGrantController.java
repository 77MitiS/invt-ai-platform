...
public class ApprovalGrantController {
    ...
    public R<Void> revoke(@PathVariable Long id,
        ...
        if (existing == null || (existing.getDeleted() != null && existing.getDeleted() == 1)) {
            throw new InvtException("err.approval.grant_not_found", 404, "grant not found");
        }
        ...
        if (!isOwner && !isAdmin) {
            throw new InvtException("err.approval.revoke_forbidden", 403,
    ...
    /**
     * Enforces the §2.4.5 6-cell matrix. Throws {@link InvtException} with
        ...
        switch (scope) {
            ...
            case ApprovalGrant.ScopeType.USER -> {
                if (body.scopeId == null || !body.scopeId.equals(String.valueOf(actorId))) {
                    throw new InvtException("err.approval.user_scope_self_only", 403,
            ...
            default -> throw new InvtException("err.approval.invalid_scope", 400,
    ...
    private void requireAdminPlusPassword(boolean isAdmin, String rawPassword, Long actorId) {
        if (!isAdmin) {
            throw new InvtException("err.approval.admin_required", 403, "admin role required");
        }
        if (rawPassword == null || rawPassword.isEmpty()) {
            throw new InvtException("err.approval.password_required", 403,
    ...
    private static void validate(CreateGrantRequest body) {
        if (body.scopeType == null || body.scopeType.isEmpty()) {
            throw new InvtException("err.approval.scope_type_required", 400, "scope_type is required");
        }
        if (body.scopeId == null || body.scopeId.isEmpty()) {
            throw new InvtException("err.approval.scope_id_required", 400, "scope_id is required");
        }
        if (body.maxSeverity == null
            ...
            throw new InvtException("err.approval.invalid_severity", 400,
        ...
        if (body.grantKind == null
            ...
            throw new InvtException("err.approval.invalid_grant_kind", 400,
        ...
        if ("UNTIL_TIMESTAMP".equals(body.grantKind) && body.expireAt == null) {
            throw new InvtException("err.approval.expire_at_required", 400,
        ...
        if ("UNTIL_CONVERSATION_END".equals(body.grantKind)
            ...
            throw new InvtException("err.approval.kind_scope_mismatch", 400,
    ...
    private Long resolveUserId(Authentication auth) {
        if (auth == null || auth.getName() == null) {
            throw new InvtException("err.auth.unauthenticated", 401, "未登录");
        }
        UserEntity user = authService.findByUsername(auth.getName());
        if (user == null) {
            throw new InvtException("err.auth.user_not_found", 404, "用户不存在");
        }
        return user.getId();
    }
