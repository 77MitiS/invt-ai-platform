...
public class PluginManager {
    ...
    public void loadPlugin(Path jarPath) throws Exception {
        ...
        try {
            ...
            if (!InvtPlugin.class.isAssignableFrom(entryClass)) {
                throw new PluginException("Entrypoint class " + manifest.getEntrypoint() +
                        " does not implement InvtPlugin");
            }
            InvtPlugin plugin = (InvtPlugin) entryClass.getDeclaredConstructor().newInstance();
    ...
    private PluginManifest readManifest(Path jarPath) throws IOException {
        try (JarFile jarFile = new JarFile(jarPath.toFile())) {
            ZipEntry entry = jarFile.getEntry("invt-plugin.json");
            if (entry == null) {
                throw new PluginException("No invt-plugin.json found in " + jarPath);
            }
