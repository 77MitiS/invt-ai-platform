...
public class SecurityStartupValidator implements ApplicationRunner {
    ...
    private static final String DEFAULT_JWT_SECRET = "Invt-JWT-Secret-Key-2024-Please-Change-In-Production";

    @Value("${invt.jwt.secret}")
    private String jwtSecret;

    @Value("${spring.h2.console.enabled:false}")
    private boolean h2ConsoleEnabled;

    @Value("${invt.cors.allowed-origins:*}")
    ...
    public void run(ApplicationArguments args) {
        ...
        if ("*".equals(corsOrigins.trim())) {
            log.warn("[Security] CORS allows all origins. Set INVT_CORS_ALLOWED_ORIGINS in production.");
            hasWarnings = true;
        }
