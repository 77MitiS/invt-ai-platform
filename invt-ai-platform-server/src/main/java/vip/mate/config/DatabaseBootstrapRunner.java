 ...
 * <ul>
 *   <li><b>Web/dev mode</b> (default, {@code invt.setup.await-language-selection=false}):
 *       auto-initializes immediately with {@code invt.setup.default-locale} (zh-CN).</li>
 *   <li><b>Desktop mode</b> ({@code invt.setup.await-language-selection=true}):
...
public class DatabaseBootstrapRunner implements ApplicationRunner {
     ...
     * <p>
     * Desktop sets this via: {@code --invt.setup.await-language-selection=true}
     */
    @Value("${invt.setup.await-language-selection:false}")
    private boolean awaitLanguageSelection;

    /** Default locale for auto-initialization. */
    @Value("${invt.setup.default-locale:zh-CN}")
