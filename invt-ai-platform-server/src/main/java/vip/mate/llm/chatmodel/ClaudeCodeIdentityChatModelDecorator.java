 ...
 *   <li><b>Brand scrub</b>: replace {@code "Invt"}/{@code "invt"}
 ...
 *       names are stripped of the {@code mcp_} prefix so Invt's tool
...
public class ClaudeCodeIdentityChatModelDecorator implements ChatModel {
    ...
    Prompt transform(Prompt original) {
        ...
        for (Message msg : source) {
            if (msg instanceof SystemMessage sm && !systemSeen) {
                ...
                // by the time Invt stores the AssistantMessage the names
    ...
    private ChatOptions transformOptions(ChatOptions options) {
        ...
        if (hasToolNames) {
            ...
            // a wider registry. If Invt populates it (most paths use callbacks
     ...
     * Invt's tool implementation never sees the prefix.
     */
