...
public class ChatGPTResponsesClient {
    ...
    public Flux<StreamEvent> streamEvents(String model, List<Message> messages, Double temperature,
        ...
        return webClient.post()
                ...
                .onStatus(status -> status.is4xxClientError() || status.is5xxServerError(),
                        response -> response.bodyToMono(String.class)
                                .map(errorBody -> {
                                    ...
                                    return new InvtException("ChatGPT API " + response.statusCode() + ": " + errorBody);
                                }))
                ...
                .onErrorMap(e -> e instanceof InvtException ? e
                        : new InvtException("err.llm.chatgpt_stream_failed", "ChatGPT 流式调用失败: " + e.getMessage()));
    }
    ...
    private StreamEvent parseSSEEvent(String eventData) {
        try {
            ...
            if ("response.failed".equals(type)) {
                ...
                throw new InvtException("err.llm.chatgpt_error", "ChatGPT 返回错误: " + error);
            }
    ...
    private void setHeaders(HttpHeaders headers, String accessToken, String accountId) {
        if (accountId == null || accountId.isBlank()) {
            throw new InvtException("err.llm.chatgpt_account_missing", "chatgpt-account-id 缺失，请断开后重新 OAuth 登录");
        }
