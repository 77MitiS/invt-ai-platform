...
public class ModelConfigController {
    ...
    /** Provider list for chat model selector — same data as admin endpoint but viewer-accessible. */
    @Operation(summary = "获取可用 Provider 列表（viewer 级别，供对话页模型选择器使用）")
    @GetMapping("/chat-providers")
    @RequireWorkspaceRole("viewer")
    public R<List<ProviderInfoDTO>> listForChat() {
        return R.ok(modelProviderService.listProviders());
    }
