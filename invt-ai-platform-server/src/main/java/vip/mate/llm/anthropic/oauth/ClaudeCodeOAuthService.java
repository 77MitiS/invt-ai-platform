 ...
 * RFC-062 PR-4. Until then, Invt piggybacks on whatever credentials the
...
public class ClaudeCodeOAuthService {
     ...
     *
     * @throws InvtException with key {@code err.anthropic.no_claude_code}
    ...
    public String getValidToken() {
        ClaudeCodeCredentials creds = reader.read()
                .orElseThrow(() -> new InvtException("err.anthropic.no_claude_code",
        ...
        if (!creds.canRefresh()) {
            throw new InvtException("err.anthropic.token_expired_no_refresh",
