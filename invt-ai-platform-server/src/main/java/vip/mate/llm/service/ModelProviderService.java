...
public class ModelProviderService {
     ...
     * {@code invt-ai-platform-ui/src/views/Settings/Models/composables/useProviderForm.ts}
     * and the routing fallback in {@code invt-ai-platform-ui/src/api/index.ts}.</p>
     */
    ...
    public ProviderInfoDTO createCustomProvider(CreateCustomProviderRequest request) {
        if (!StringUtils.hasText(request.getId()) || !StringUtils.hasText(request.getName())) {
            throw new InvtException("err.llm.provider_fields_required", "Provider id 和名称不能为空");
        }
        ...
        if (!PROVIDER_ID_PATTERN.matcher(request.getId()).matches()) {
            throw new InvtException("err.llm.provider_id_invalid",
                    "Provider id 仅允许字母/数字及 . _ -（不允许斜杠或空格），首字符必须是字母或数字，长度 1-64: " + request.getId());
        }
        if (modelProviderMapper.selectById(request.getId()) != null) {
            throw new InvtException("err.llm.provider_exists", "Provider 已存在: " + request.getId());
        }
    ...
    public void deleteCustomProvider(String providerId) {
        ...
        if (!Boolean.TRUE.equals(provider.getIsCustom())) {
            throw new InvtException("err.llm.provider_builtin_readonly", "内置 Provider 不支持删除");
        }
    ...
    private ModelProviderEntity getProvider(String providerId) {
        ...
        if (provider == null) {
            throw new InvtException("err.llm.provider_not_found", "Provider 不存在: " + providerId);
        }
        return provider;
    }
