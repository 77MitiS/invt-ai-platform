 ...
 *       127.0.0.1:1455 to receive the callback. Used when Invt is reached via
 ...
 * <p>Mode selection:
 * <ol>
 *   <li>Config override {@code invt.oauth.openai.deployment-mode}: one of
...
public class OpenAIOAuthService {
     ...
     * Invt on the same machine they'll do the OAuth login on — LOCAL works.
     * Any other host (a domain, a public IP, a private LAN IP) means the user's
     * browser cannot resolve {@code localhost:1455} to Invt's server, so we
    ...
    OAuthFlowMode resolveFlowMode(String requestHost) {
        String configMode = System.getProperty("invt.oauth.openai.deployment-mode",
                System.getenv("INVT_OAUTH_OPENAI_DEPLOYMENT_MODE"));
        if (configMode != null) {
     ...
     * their browser's address bar back into Invt. We parse code + state and
    ...
    public void completeFromPastedUrl(String pastedUrl) {
        if (pastedUrl == null || pastedUrl.isBlank()) {
            throw new InvtException("err.llm.oauth_paste_empty",
                    "粘贴的 URL 为空，请回到浏览器地址栏复制完整 URL");
        }
        String trimmed = pastedUrl.trim();
        int q = trimmed.indexOf('?');
        if (q < 0) {
            throw new InvtException("err.llm.oauth_paste_invalid",
        ...
        if (code == null || code.isBlank()) {
            throw new InvtException("err.llm.oauth_paste_no_code",
                    "粘贴的 URL 中缺少 code 参数，登录可能未完成");
        }
        if (state == null || state.isBlank()) {
            throw new InvtException("err.llm.oauth_paste_no_state",
    ...
    private void exchangeToken(String code, String state) {
        ...
        if (codeVerifier == null) {
            throw new InvtException("err.llm.oauth_state_invalid", "无效的 OAuth state，可能已过期或重复使用");
        }
    ...
    public void refreshToken() {
        ...
        if (!StringUtils.hasText(provider.getOauthRefreshToken())) {
            throw new InvtException("err.llm.oauth_no_refresh", "无 refresh_token，请重新登录");
        }
    ...
    public String ensureValidAccessToken() {
        ...
        if (!StringUtils.hasText(provider.getOauthAccessToken())) {
            throw new InvtException("err.llm.oauth_not_connected", "未连接 OpenAI OAuth，请先登录");
        }
    ...
    private JsonNode postTokenRequest(String formBody) {
        try {
            ...
            throw new InvtException("err.llm.oauth_exchange_failed", "OAuth token 交换失败: " + e.getMessage());
        }
    }
    ...
    private void saveTokens(JsonNode tokenResponse) {
        ...
        if (!StringUtils.hasText(accessToken)) {
            throw new InvtException("err.llm.oauth_no_token", "OAuth 响应中缺少 access_token");
        }
    ...
    private ModelProviderEntity getProvider() {
        ModelProviderEntity provider = modelProviderMapper.selectById(PROVIDER_ID);
        if (provider == null) {
            throw new InvtException("err.llm.chatgpt_not_configured", "OpenAI ChatGPT provider 未配置，请检查数据库初始化");
        }
        return provider;
    }
    ...
    String resolveCallbackBindHost() {
        String configured = System.getProperty("invt.oauth.openai.callback-bind-host",
                System.getenv("INVT_OAUTH_OPENAI_CALLBACK_BIND_HOST"));
    ...
    private String generateCodeChallenge(String codeVerifier) {
        try {
            ...
            throw new InvtException("err.llm.pkce_failed", "PKCE code_challenge 生成失败: " + e.getMessage());
        }
    }
