 ...
 *
 * <p>Used when Invt runs on a remote host and the browser cannot reach
...
public class OpenAIDeviceCodeService {
    ...
    @Value("${invt.oauth.openai.device.poll-min-interval-ms:3000}")
    private long pollMinIntervalMs;

    @Value("${invt.oauth.openai.device.session-ttl-seconds:900}")
    private long defaultSessionTtlSeconds;

    @Value("${invt.oauth.openai.device.user-agent:" + DEFAULT_USER_AGENT + "}")
    ...
    public DeviceCodeStartResult start() {
        ...
        try {
            ...
            throw new InvtException("err.llm.device_code_start_failed",
