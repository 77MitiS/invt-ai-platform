...
public class CronJobService implements ApplicationRunner {
    ...
    public CronJobDTO getById(Long id, Long workspaceId) {
        ...
        if (entity == null) {
            throw new InvtException("err.cron.not_found", "定时任务不存在: " + id);
        }
    ...
    public CronJobDTO update(Long id, CronJobDTO dto, Long workspaceId) {
        ...
        if (newName != null && newAgentId != null) {
            ...
            if (collision != null) {
                throw new InvtException("err.cron.duplicate_name",
    ...
    private String toSpringCron(String cron) {
        ...
        if (parts.length != 5) {
            throw new InvtException("Cron 表达式必须是 5 字段（分 时 日 月 周）");
        }
        // 标准化 day-of-week
        ...
        try {
            ...
            throw new InvtException("Cron 表达式非法: " + e.getMessage());
        }
        return springCron;
    }
    ...
    private void validateDto(CronJobDTO dto) {
        if (dto.getName() == null || dto.getName().isBlank()) {
            throw new InvtException("err.cron.name_required", "任务名称不能为空");
        }
        if (dto.getCronExpression() == null || dto.getCronExpression().isBlank()) {
            throw new InvtException("err.cron.expression_required", "Cron 表达式不能为空");
        }
        ...
        if (!"wiki_process".equals(taskType) && dto.getAgentId() == null) {
            throw new InvtException("err.cron.agent_required", "请选择关联 Agent");
        }
        ...
        if (("text".equals(taskType) || "reminder".equals(taskType))
            ...
            throw new InvtException("err.cron.trigger_required", "触发消息不能为空");
        }
        if ("agent".equals(taskType) && (dto.getRequestBody() == null || dto.getRequestBody().isBlank())) {
            throw new InvtException("err.cron.target_required", "执行目标不能为空");
        }
    ...
    private void validateWikiProcessPayload(String requestBody) {
        if (requestBody == null || requestBody.isBlank()) {
            throw new InvtException("err.cron.wiki_kb_required", "请选择知识库");
        }
        Long kbId;
        try {
            ...
            if (v.isNumber()) {
                ...
                try {
                    ...
                    throw new InvtException("err.cron.wiki_kb_invalid", "知识库 ID 格式不合法");
                }
            } else {
                throw new InvtException("err.cron.wiki_kb_invalid", "知识库 ID 格式不合法");
            }
            ...
            throw new InvtException("err.cron.wiki_kb_invalid", "知识库参数解析失败");
        }
        WikiKnowledgeBaseEntity kb = wikiKnowledgeBaseMapper.selectById(kbId);
        if (kb == null) {
            throw new InvtException("err.cron.wiki_kb_not_found", "知识库不存在");
        }
    }
}
