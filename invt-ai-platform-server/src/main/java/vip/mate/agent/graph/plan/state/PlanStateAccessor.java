 ...
 * <p>
 * 参照 {@link vip.mate.agent.graph.state.InvtStateAccessor} 的模式，
...
public final class PlanStateAccessor {
    ...
    public String systemPrompt() {
        return state.value(InvtStateKeys.SYSTEM_PROMPT, "你是一个有帮助的AI助手。");
    }

    public String conversationId() {
        return state.value(InvtStateKeys.CONVERSATION_ID, "");
    }

    public String traceId() {
        return state.value(InvtStateKeys.TRACE_ID, "");
    }

    /**
     * The {@link vip.mate.agent.context.ChatOrigin} forwarded into graph
     * state by {@code InvtStateAccessor.OutputBuilder.chatOrigin}.
    ...
    public vip.mate.agent.context.ChatOrigin chatOrigin() {
        return state.<vip.mate.agent.context.ChatOrigin>value(InvtStateKeys.CHAT_ORIGIN)
                .orElse(vip.mate.agent.context.ChatOrigin.EMPTY);
    }

    // ===== 会话消息（复用 InvtStateKeys.MESSAGES）=====

    @SuppressWarnings("unchecked")
    public List<Message> messages() {
        return state.<List<Message>>value(InvtStateKeys.MESSAGES).orElse(List.of());
    }
    ...
    public static final class OutputBuilder {
        ...
        // ---- 会话消息（写入共享键 InvtStateKeys.MESSAGES）----
        public OutputBuilder messages(List<Message> msgs) {
            return put(InvtStateKeys.MESSAGES, msgs);
        }
        ...
        public OutputBuilder contentStreamed(boolean streamed) {
            return put(InvtStateKeys.CONTENT_STREAMED, streamed);
        }

        public OutputBuilder thinkingStreamed(boolean streamed) {
            return put(InvtStateKeys.THINKING_STREAMED, streamed);
        }

        // ---- 事件流（写入共享键 InvtStateKeys.PENDING_EVENTS）----
        public OutputBuilder events(List<GraphEventPublisher.GraphEvent> events) {
            return put(InvtStateKeys.PENDING_EVENTS, events);
        }

        // ---- 阶段标记（写入共享键 InvtStateKeys.CURRENT_PHASE）----
        public OutputBuilder currentPhase(String phase) {
            return put(InvtStateKeys.CURRENT_PHASE, phase);
        }
        ...
        public OutputBuilder mergeUsage(OverAllState currentState,
            ...
            int existingPrompt = currentState.value(InvtStateKeys.PROMPT_TOKENS, 0);
            int existingCompletion = currentState.value(InvtStateKeys.COMPLETION_TOKENS, 0);
            int existingLlmCalls = currentState.value(InvtStateKeys.LLM_CALL_COUNT, 0);
            map.put(InvtStateKeys.PROMPT_TOKENS, existingPrompt + result.promptTokens());
            map.put(InvtStateKeys.COMPLETION_TOKENS, existingCompletion + result.completionTokens());
            map.put(InvtStateKeys.LLM_CALL_COUNT, existingLlmCalls + 1);
            return this;
        }
