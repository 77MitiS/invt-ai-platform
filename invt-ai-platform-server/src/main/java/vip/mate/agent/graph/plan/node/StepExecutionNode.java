...
public class StepExecutionNode implements NodeAction {
    ...
    public Map<String, Object> apply(OverAllState state) throws Exception {
        ...
        String conversationId = state.value(InvtStateKeys.CONVERSATION_ID, "");
        String agentId = state.value(InvtStateKeys.AGENT_ID, "");
        String workspaceBasePath = state.value(InvtStateKeys.WORKSPACE_BASE_PATH, "");
        ...
        vip.mate.agent.context.ChatOrigin chatOrigin =
                state.<vip.mate.agent.context.ChatOrigin>value(InvtStateKeys.CHAT_ORIGIN)
                        .orElse(vip.mate.agent.context.ChatOrigin.EMPTY);
        String runtimeModelName = state.value(InvtStateKeys.RUNTIME_MODEL_NAME, "");
        String runtimeProviderId = state.value(InvtStateKeys.RUNTIME_PROVIDER_ID, "");
        ...
        try {
            while (toolCallCount < MAX_TOOL_CALLS_PER_STEP) {
                ...
                String preApprovedPayload = state.value(InvtStateKeys.PRE_APPROVED_TOOL_CALL, "");
            ...
            if (approvalTriggered) {
                ...
                return PlanStateAccessor.output()
                        ...
                        .put(InvtStateKeys.PROMPT_TOKENS, state.value(InvtStateKeys.PROMPT_TOKENS, 0) + stepPromptTokens)
                        .put(InvtStateKeys.COMPLETION_TOKENS, state.value(InvtStateKeys.COMPLETION_TOKENS, 0) + stepCompletionTokens)
                        .events(events)
                        .build();
            }
            ...
            if (!stepDirectOutputs.isEmpty()) {
                ...
                return PlanStateAccessor.output()
                        ...
                        .put(InvtStateKeys.RETURN_DIRECT_TRIGGERED, true)
                        .put(InvtStateKeys.DIRECT_TOOL_OUTPUTS, List.copyOf(stepDirectOutputs))
                        .put(InvtStateKeys.PROMPT_TOKENS,
                                state.value(InvtStateKeys.PROMPT_TOKENS, 0) + stepPromptTokens)
                        .put(InvtStateKeys.COMPLETION_TOKENS,
                                state.value(InvtStateKeys.COMPLETION_TOKENS, 0) + stepCompletionTokens)
                        .events(events)
                        .build();
            }
