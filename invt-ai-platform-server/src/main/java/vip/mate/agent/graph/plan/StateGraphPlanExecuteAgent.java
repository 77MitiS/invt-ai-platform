...
public class StateGraphPlanExecuteAgent extends BaseAgent implements StructuredStreamCapable {
    ...
    public Flux<AgentService.StreamDelta> chatStructuredStream(String userMessage, String conversationId,
        ...
        try {
            ...
            inputs.put(InvtStateKeys.REQUESTER_ID, requesterId != null ? requesterId : "");
        ...
        try {
            ...
            if (ctx != null) {
                ...
                if (!ctx.completedResults().isEmpty()) {
                    ...
                    List<Message> messages = (List<Message>) inputs.get(InvtStateKeys.MESSAGES);
            ...
            if (toolCallPayload != null && !toolCallPayload.isEmpty()) {
                inputs.put(InvtStateKeys.PRE_APPROVED_TOOL_CALL, toolCallPayload);
            }
        ...
        return BaseAgent.routingStartupDelta(inputs).concatWith(compiledGraph.stream(inputs, config)
                .flatMapIterable(output -> {
                    ...
                    boolean contentAlreadyStreamed = output.state()
                            .value(InvtStateKeys.CONTENT_STREAMED, false);
                    boolean thinkingAlreadyStreamed = output.state()
                            .value(InvtStateKeys.THINKING_STREAMED, false);
                    ...
                    finalPromptTokens.set(output.state().value(InvtStateKeys.PROMPT_TOKENS, 0));
                    finalCompletionTokens.set(output.state().value(InvtStateKeys.COMPLETION_TOKENS, 0));
                    finalModelName.set(output.state().value(InvtStateKeys.RUNTIME_MODEL_NAME, ""));
                    finalProviderId.set(output.state().value(InvtStateKeys.RUNTIME_PROVIDER_ID, ""));
    ...
    private Map<String, Object> buildInitialState(String userMessage, String conversationId) {
        ...
        inputs.put(InvtStateKeys.SYSTEM_PROMPT,
                systemPrompt != null ? systemPrompt : "你是一个有帮助的AI助手。");
        inputs.put(InvtStateKeys.CONVERSATION_ID, conversationId);
        inputs.put(InvtStateKeys.AGENT_ID, agentId != null ? agentId : "");
        inputs.put(InvtStateKeys.WORKSPACE_BASE_PATH, workspaceBasePath != null ? workspaceBasePath : "");
        // 注入会话消息（复用 InvtStateKeys.MESSAGES，与 ReAct 一致）
        inputs.put(InvtStateKeys.MESSAGES, messages);
        // 注入 working context
        inputs.put(PlanStateKeys.WORKING_CONTEXT, workingContext);
        inputs.put(PlanStateKeys.CURRENT_STEP_INDEX, 0);
        inputs.put(InvtStateKeys.CONTENT_STREAMED, false);
        inputs.put(InvtStateKeys.THINKING_STREAMED, false);
        inputs.put(InvtStateKeys.STREAMED_CONTENT, "");
        inputs.put(InvtStateKeys.STREAMED_THINKING, "");
        inputs.put(InvtStateKeys.REQUESTER_ID, "");
        inputs.put(InvtStateKeys.PROMPT_TOKENS, 0);
        inputs.put(InvtStateKeys.COMPLETION_TOKENS, 0);
        inputs.put(InvtStateKeys.RUNTIME_MODEL_NAME, modelName != null ? modelName : "");
        inputs.put(InvtStateKeys.RUNTIME_PROVIDER_ID, runtimeProviderId != null ? runtimeProviderId : "");
        inputs.put(InvtStateKeys.TRACE_ID, UUID.randomUUID().toString().substring(0, 8));
        ...
        if (currentTurn.routingDecision() != null
            ...
            inputs.put(InvtStateKeys.ROUTING_DECISION, currentTurn.routingDecision().toMap());
        }
        ...
        inputs.put(InvtStateKeys.CHAT_ORIGIN, origin);
        ...
        if (goalService != null && conversationId != null && !conversationId.isBlank()) {
            try {
                ...
                if (active != null) {
                    inputs.put(InvtStateKeys.ACTIVE_GOAL, active);
                }
            } catch (Exception e) {
                log.warn("[{}] findActiveByConversation failed: {}", agentName, e.getMessage());
            }
        }
        inputs.put(InvtStateKeys.GOAL_EVALUATED_THIS_RUN, false);
        inputs.put(InvtStateKeys.GOAL_FOLLOWUP_INJECTED, false);
        inputs.put(InvtStateKeys.GOAL_FOLLOWUP_PROMPT, "");
