...
import static vip.mate.agent.graph.state.InvtStateKeys.*;
...
public class ReasoningNode implements NodeAction {
    ...
    private static InvtStateAccessor.OutputBuilder reasonOutput() {
        return InvtStateAccessor.output();
    }
    ...
    public Map<String, Object> apply(OverAllState state) throws Exception {
        InvtStateAccessor accessor = new InvtStateAccessor(state);
        ...
        String workspaceBasePath = state.value(vip.mate.agent.graph.state.InvtStateKeys.WORKSPACE_BASE_PATH, "");
        String agentIdStr = state.value(InvtStateKeys.AGENT_ID, "");
        String userMsg = state.value(InvtStateKeys.USER_MESSAGE, "");
        ...
        try {
            ...
            if (result.isPromptTooLong() && conversationWindowManager != null) {
                ...
                // InvtStateAccessor.agentId() returns String per state
     ...
     * iteration-boundary toggle ({@code invt.stream.iteration-events})
