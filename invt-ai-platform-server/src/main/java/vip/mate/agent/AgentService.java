...
public class AgentService {
    ...
    public AgentEntity getAgent(Long id) {
        ...
        if (entity == null) {
            throw new InvtException("err.agent.not_found", "Agent不存在: " + id);
        }
        return entity;
    }
     ...
     * interceptor in {@code invt-ai-platform-ui/src/api/index.ts}); the 409 lives in
    ...
    private void requireUniqueName(AgentEntity agent, Long excludeId) {
        if (agent.getName() == null || agent.getName().isBlank()) {
            throw new InvtException("err.agent.name_required", 400, "Agent 名称不能为空");
        }
        ...
        if (count != null && count > 0) {
            throw new InvtException("err.agent.duplicate_name", 409,
        ...
        return agentInstances
                ...
                .computeIfAbsent(modelKey, key -> {
                    ...
                    if (!Boolean.TRUE.equals(entity.getEnabled())) {
                        throw new InvtException("err.agent.disabled", "Agent 已禁用: " + entity.getName());
                    }
