...
public class AgentController {
    ...
    public R<Void> delete(@PathVariable Long id,
        ...
        if (!systemAdmin && !workspaceAdmin && !isCreator) {
            throw new InvtException("err.agent.delete_forbidden", 403,
    ...
    private void verifyResourceWorkspace(Long resourceWorkspaceId, Long headerWorkspaceId) {
        ...
        if (resourceWorkspaceId != null && !resourceWorkspaceId.equals(requestedWs)) {
            throw new InvtException("err.common.wrong_workspace", 403, "资源不属于当前工作区");
        }
    }
    ...
    private void verifyAgentEnabled(AgentEntity agent) {
        if (agent != null && !Boolean.TRUE.equals(agent.getEnabled())) {
            throw new InvtException("err.agent.disabled", "Agent 已禁用: " + agent.getName());
        }
    }

    private Long resolveUserId(Authentication auth) {
        if (auth == null) {
            throw new InvtException("err.auth.unauthenticated", 401, "Not authenticated");
        }
        UserEntity user = authService.findByUsername(auth.getName());
        if (user == null) {
            throw new InvtException("err.auth.user_not_found", 401, "User not found: " + auth.getName());
        }
        return user.getId();
    }
