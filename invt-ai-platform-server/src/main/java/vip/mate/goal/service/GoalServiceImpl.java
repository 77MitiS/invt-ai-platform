...
public class GoalServiceImpl implements GoalService {
    ...
    public GoalEntity create(GoalCreateRequest req, String username) {
        ...
        if (active != null) {
            throw new InvtException("err.goal.conversation_has_active", 409,
    ...
    public GoalEntity getById(Long id) {
        ...
        if (g == null) {
            throw new InvtException("err.goal.not_found", 404, "Goal not found: " + id);
        }
        return g;
    }
    ...
    public List<GoalEntity> list(String status, String username, int limit) {
        ...
        if (status != null && !status.isBlank()) {
            ...
            try {
                ...
                throw new InvtException("err.goal.bad_status", 400, "Unknown status: " + status);
            }
    ...
    public GoalEntity appendCriterion(Long id, String criterion, String username) {
        if (criterion == null || criterion.isBlank()) {
            throw new InvtException("err.goal.criterion_empty", 400, "Criterion must not be empty");
        }
    ...
    private void validateCreate(GoalCreateRequest req) {
        if (req == null) {
            throw new InvtException("err.goal.bad_request", 400, "Request body required");
        }
        if (req.getConversationId() == null || req.getConversationId().isBlank()) {
            throw new InvtException("err.goal.bad_request", 400, "conversationId required");
        }
        if (req.getAgentId() == null) {
            throw new InvtException("err.goal.bad_request", 400, "agentId required");
        }
        if (req.getWorkspaceId() == null) {
            throw new InvtException("err.goal.bad_request", 400, "workspaceId required");
        }
        if (req.getTitle() == null || req.getTitle().isBlank()) {
            throw new InvtException("err.goal.bad_request", 400, "title required");
        }
        if (req.getTitle().length() > 255) {
            throw new InvtException("err.goal.bad_request", 400, "title too long (>255)");
        }
        if (req.getTurnBudget() != null) validateBudget(req.getTurnBudget(), "turnBudget");
        if (req.getLlmCallBudget() != null) validateBudget(req.getLlmCallBudget(), "llmCallBudget");
    }
    ...
    private static void validateBudget(int v, String name) {
        if (v <= 0) {
            throw new InvtException("err.goal.invalid_budget", 400,
    ...
    private void ensureNotTerminal(GoalEntity g, String op) {
        if (g.getStatus().isTerminal()) {
            throw new InvtException("err.goal.terminal_state", 409,
    ...
    private GoalEntity retryOptimistic(Long id, String op, UpdateBuilder builder) {
        for (int i = 0; i < OPTIMISTIC_LOCK_MAX_RETRIES; i++) {
            ...
            if (fresh == null) {
                throw new InvtException("err.goal.not_found", 404,
                        "Goal not found: " + id);
            }
        ...
        throw new InvtException("err.goal.optimistic_lock_conflict", 409,
    ...
    private GoalEntity flipStatus(Long id, GoalStatus from, GoalStatus to,
        ...
        GoalEntity g = retryOptimistic(id, to.getValue(), fresh -> {
            if (fresh.getStatus() != from) {
                throw new InvtException("err.goal.bad_transition", 409,
