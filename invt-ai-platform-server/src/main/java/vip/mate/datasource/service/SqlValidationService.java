...
public class SqlValidationService {
    ...
    public String validateAndNormalize(String sql) {
        if (sql == null || sql.isBlank()) {
            throw new InvtException("err.datasource.sql_empty", "SQL 不能为空");
        }
        ...
        try {
            ...
            if (stmts.getStatements().size() != 1) {
                throw new InvtException("err.datasource.only_single_sql", "仅允许执行单条 SQL 语句，检测到 " + stmts.getStatements().size() + " 条");
            }
            statement = stmts.getStatements().get(0);
        } catch (JSQLParserException e) {
            throw new InvtException("err.datasource.sql_parse_failed", "SQL 解析失败: " + e.getMessage());
        }

        // 仅允许 SELECT
        if (!(statement instanceof Select)) {
            throw new InvtException("err.datasource.only_select", "仅允许 SELECT 查询，检测到: " + statement.getClass().getSimpleName());
        }
