...
public class WorkspaceService {
    ...
    public WorkspaceEntity getById(Long id) {
        ...
        if (entity == null) {
            throw new InvtException("err.workspace.not_found", "工作区不存在: " + id);
        }
        return entity;
    }
    ...
    public WorkspaceEntity create(WorkspaceEntity entity, Long creatorUserId) {
        ...
        if (getBySlug(entity.getSlug()) != null) {
            throw new InvtException("err.workspace.slug_exists", "工作区标识已存在: " + entity.getSlug());
        }
    ...
    public WorkspaceEntity update(WorkspaceEntity entity) {
        ...
        if (DEFAULT_SLUG.equals(existing.getSlug()) && !DEFAULT_SLUG.equals(entity.getSlug())) {
            throw new InvtException("err.workspace.cannot_modify_default", "不能修改默认工作区的标识");
        }
    ...
    public void delete(Long id) {
        ...
        if (DEFAULT_SLUG.equals(existing.getSlug())) {
            throw new InvtException("err.workspace.cannot_delete_default", "不能删除默认工作区");
        }
        ...
        if (kbCount > 0) {
            throw new InvtException("err.workspace.not_empty", 409,
    ...
    public WorkspaceMemberEntity addMember(Long workspaceId, Long userId, String role) {
        ...
        if (existing != null) {
            throw new InvtException("err.workspace.member_exists", 409, "用户已经是该工作区的成员");
        }
        WorkspaceMemberEntity member = new WorkspaceMemberEntity();
    ...
    public WorkspaceMemberEntity updateMemberRole(Long workspaceId, Long userId, String role) {
        WorkspaceMemberEntity member = getMembership(workspaceId, userId);
        if (member == null) {
            throw new InvtException("err.workspace.not_member", 404, "用户不是该工作区的成员");
        }
        if ("owner".equals(member.getRole())) {
            throw new InvtException("err.workspace.cannot_modify_owner", 400, "不能修改工作区拥有者的角色");
        }
        member.setRole(normalizeAssignableRole(role));
    ...
    private String normalizeAssignableRole(String role) {
        ...
        return switch (normalized) {
            case "super_admin", "admin", "member", "viewer" -> normalized;
            case "owner" -> throw new InvtException(
                    "err.workspace.invalid_member_role", 400, "不能通过成员管理授予 owner 角色");
            default -> throw new InvtException(
    ...
    public void removeMember(Long workspaceId, Long userId) {
        ...
        if ("owner".equals(member.getRole())) {
            throw new InvtException("err.workspace.cannot_remove_owner", 400, "不能移除工作区拥有者");
        }
    ...
    public void requirePermission(Long workspaceId, Long userId, String minRole) {
        if (!hasPermission(workspaceId, userId, minRole)) {
            throw new InvtException("err.workspace.insufficient_permission", 403, "权限不足：需要 " + minRole + " 或更高角色");
        }
    }
