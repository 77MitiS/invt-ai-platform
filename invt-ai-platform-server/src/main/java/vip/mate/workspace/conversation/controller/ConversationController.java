...
public class ConversationController {
     ...
     * 提交消息评价反馈（点赞 / 点踩）
     */
    @Operation(summary = "提交消息评价反馈")
    @PutMapping("/{conversationId}/messages/{messageId}/feedback")
    public R<Void> submitFeedback(@PathVariable String conversationId,
        ...
        if (rating == null || (!rating.equals("like") && !rating.equals("dislike"))) {
            return R.fail("rating 必须为 like 或 dislike");
        }
        try {
            conversationService.submitFeedback(conversationId, messageId, rating);
            return R.ok();
