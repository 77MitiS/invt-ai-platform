...
public class ConversationService {
    ...
    /**
     * Submit user feedback (thumbs up / thumbs down) on a message.
     * Stores the rating in the message's metadata JSON.
     */
    public void submitFeedback(String conversationId, Long messageId, String rating) {
        MessageEntity message = messageMapper.selectOne(new LambdaQueryWrapper<MessageEntity>()
        ...
        // Update metadata JSON: merge feedback field
        Map<String, Object> meta;
        try {
            meta = objectMapper.readValue(
                    message.getMetadata() != null ? message.getMetadata() : "{}",
                    new TypeReference<Map<String, Object>>() {});
        } catch (Exception e) {
            meta = new java.util.HashMap<>();
        }
        meta.put("feedback", rating);
        meta.put("feedbackAt", System.currentTimeMillis());
        try {
            message.setMetadata(objectMapper.writeValueAsString(meta));
        } catch (Exception e) {
            log.warn("Failed to serialize feedback metadata: {}", e.getMessage());
        }
        messageMapper.updateById(message);
        log.info("Feedback submitted: conv={}, msg={}, rating={}", conversationId, messageId, rating);
    }
}
