...
public class TriggerEventIngestService {
    ...
    @Value("${invt.workflow.trigger.async-dispatch:true}")
    private boolean asyncDispatch;

    @Value("${invt.workflow.trigger.dispatch-pool-size:8}")
    private int dispatchPoolSize;

    @Value("${invt.workflow.trigger.dispatch-queue-capacity:256}")
    ...
    private IngestResult processSingle(TriggerEntity trigger, TriggerEventEnvelope envelope) {
        ...
        // explicitly opts out via invt.workflow.trigger.async-dispatch=false.
     ...
     * {@code invt.workflow.trigger.dedup-sweep-interval-ms}. The
     * initial delay matches the cadence so a JVM that just started doesn't
     * race {@code recordDedupRow} for the same window.
     */
    @Scheduled(
            fixedDelayString = "${invt.workflow.trigger.dedup-sweep-interval-ms:300000}",
            initialDelayString = "${invt.workflow.trigger.dedup-sweep-initial-delay-ms:300000}")
