...
import { parsePrompt } from '@/utils/agentPromptProfile'
...
const suggestions = computed(() => {
  ...
  if (!agent) return []
  // Parse agent's systemPrompt to extract role + goal
  const profile = parsePrompt(agent.systemPrompt)
  const role = (profile.role || '').trim()
  const goal = (profile.goal || '').trim()
  const name = agent.name || ''
  if (role || goal) {
    // Generate 4 personalized suggestion prompts
    const prompts: string[] = []
    if (role) {
      prompts.push(`你作为${role}，请介绍一下你能帮我做什么？`)
    }
    if (goal) {
      prompts.push(`请根据你的目标「${goal}」，给我一个具体的执行方案`)
    }
    prompts.push('最近有什么新的想法或建议可以分享吗？')
    prompts.push('帮我分析一下这个领域的关键趋势和机会')
...
// 思考模式：只有两个状态 — 开或关
const thinkingEnabled = ref(localStorage.getItem('invt_thinking') !== 'off')
const thinkingLevel = computed(() => thinkingEnabled.value ? 'high' : 'off')
watch(thinkingEnabled, (v) => localStorage.setItem('invt_thinking', v ? 'on' : 'off'))
...
onMounted(async () => {
  ...
  // Refresh agents list when user returns to this tab (e.g. after deleting in Agents page)
