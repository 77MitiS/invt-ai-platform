...
const listFilters = computed<{ key: ListFilterKey; label: string; count: number }[]>(() => [
  { key: 'all', label: t('enterprise.account.filterAll'), count: accounts.value.length },
  { key: 'hot', label: t('enterprise.account.filterHot'), count: accounts.value.filter(a => a.score >= 70).length },
  { key: 'cooling', label: t('enterprise.account.filterCooling'), count: accounts.value.filter(a => a.score < 50).length },
])
...
onMounted(async () => {
  try {
    const res: any = await enterpriseApi.getAccounts()
        const data = res.data || []
        accounts.value = []
        if (Array.isArray(data) && data.length) {
      accounts.value = data.map((a: any) => ({
        id: a.id, name: a.name, short: a.short, industry: a.industry, region: a.region,
        stage: a.stage || 'prospect', aum: a.aum || '', score: a.score || 0,
        scoreDelta: '', color: a.color || '#666',
        people: (a.people || a.keyPeople || []).map((p: any) => ({ name: p.name, role: p.title || p.role, kind: p.kind || 'influencer', short: (p.name || '?')[0] })),
        events: (a.events || a.timeline || []).map((e: any) => ({ id: e.date || '', kind: e.kind || 'news', summary: e.title || e.summary || '', source: e.evidence || '', at: e.date || '' })),
        actions: (a.actions || []).map((act: any) => ({ priority: act.priority || 'medium', when: '', title: act.action || act.title || '', rationale: act.detail || '' })),
      }))
    }
  } catch { /* keep empty */ }
})
...
const filteredAccounts = computed(() => {
  ...
  switch (listFilter.value) {
    case 'hot': return list.filter(a => a.score >= 70)
    case 'cooling': return list.filter(a => a.score < 50)
    default: return list
  }
})

const selectedId = ref<string>('')
const selectedAccount = computed(() => accounts.value.find(a => a.id === selectedId.value) ?? accounts.value[0])

// Auto-select first account when data loads
watch(accounts, (val) => { if (val.length && !selectedId.value) selectedId.value = val[0].id })
