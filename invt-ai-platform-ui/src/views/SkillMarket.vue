    ...
    <div class="mc-page-frame">
      <div class="mc-page-inner skills-page">
        ...
        <div v-for="section in skillSections" :key="section.key" class="skill-section">
            ...
            <div class="skill-footer" @click.stop>
              ...
              <div class="skill-actions">
                ...
                <button
                  ...
                  class="skill-btn"
                  :title="t('skills.actions.export') || '导出'"
                  @click.stop="handleExportSkillFromCard(skill)"
                >
                  <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                ...
                <button
                  v-if="!isSkillRowVirtual(skill)"
                  class="skill-btn"
                  :title="t('skills.actions.share') || '分享'"
                  @click.stop="handleShareSkillFromCard(skill)"
                >
                  <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
          ...
          <!-- Per-section pagination — Invt frosted-pill component. -->
          ...
          <div class="detail-block">
            <div class="detail-block-head">
              ...
              <button
                v-if="!isVirtualSkill && !editingIdentity"
                class="detail-edit-btn"
                @click="handleShareSkill"
                :title="t('skills.actions.share')"
              >
                <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                  <circle cx="18" cy="5" r="3"/>
                  <circle cx="6" cy="12" r="3"/>
...
async function handleExportSkillFromCard(skill: any) {
  try {
    const res = await skillApi.exportConfig(skill.id)
    const data = res.data || res
    const blob = new Blob([JSON.stringify(data, null, 2)], { type: 'application/json' })
    const url = URL.createObjectURL(blob)
    const a = document.createElement('a')
    a.href = url
    a.download = `${skill.name || 'skill'}-config.json`
    a.click()
...
async function handleShareSkillFromCard(skill: any) {
  try {
    const res = await skillApi.share(skill.id)
    const data = res.data || res
    const link = `${window.location.origin}/skills/shared/${data.token}`
    await copyToClipboard(link)
    mcToast.success(`分享链接已复制: ${data.token}`)
  } catch {
    mcToast.error('分享失败')
  }
}
...
async function handleExportSkill() {
  if (!detailSkill.value) return
  try {
    const res = await skillApi.exportConfig(detailSkill.value.id)
    const data = res.data || res
    const blob = new Blob([JSON.stringify(data, null, 2)], { type: 'application/json' })
    const url = URL.createObjectURL(blob)
    const a = document.createElement('a')
    a.href = url
    a.download = `${detailSkill.value.name || 'skill'}-config.json`
    a.click()
    URL.revokeObjectURL(url)
    mcToast.success(t('skills.messages.exported') || '已导出')
  } catch {
    mcToast.error(t('skills.messages.exportFailed') || '导出失败')
  }
}
...
async function handleShareSkill() {
  if (!detailSkill.value) return
  try {
    const res = await skillApi.share(detailSkill.value.id)
    const data = res.data || res
    const link = `${window.location.origin}/skills/shared/${data.token}`
    await copyToClipboard(link)
    mcToast.success(`分享链接已复制: ${data.token}`)
  } catch {
    mcToast.error(t('skills.messages.shareFailed') || '分享失败')
  }
}
