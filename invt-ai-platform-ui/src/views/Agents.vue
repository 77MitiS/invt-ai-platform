  ...
  <div class="mc-page-shell">
    <div class="mc-page-frame">
      <div class="mc-page-inner agents-page">
        ...
        <!-- Agent card grid — grouped by category -->
        <div class="agent-grid" v-if="filteredAgents.length > 0">
          <template v-for="group in groupedAgents" :key="group.category">
            <div class="category-header">
              <span class="category-header__icon">{{ group.icon }}</span>
              <h2 class="category-header__name">{{ group.category }}</h2>
              <span class="category-header__count">{{ group.agents.length }}</span>
            </div>
            <div
        ...
        <!-- Pagination -->
        <div v-if="filteredAgents.length > pageSize" class="agent-pagination">
          <McPagination
            v-model:page="page"
        ...
        <!-- Empty state -->
        <div v-if="filteredAgents.length === 0" class="empty-state mc-surface-card">
...
const filteredAgents = computed(() => {
  ...
  if (activeTags.value.length) {
    list = list.filter(a => {
      ...
      return activeTags.value.some(t => tags.includes(t))
    })
  }
  return list
})

// Group agents by their first category tag for categorized display.
// Categories are ordered logically; uncategorized agents go last.
const categoryOrder = [
  '工程开发', '设计', '产品', '财务管理', '市场营销', '销售', '测试',
  '专业化', '人力资源', '法律', '学术研究', '项目管理', '支持服务',
  '供应链', '游戏开发', '付费媒体', '空间计算',
  'Unreal', 'Unity', 'Godot', 'Roblox', 'Blender',
  'MCP记忆',
]
const categoryIcons: Record<string, string> = {
  ...
  '人力资源': '👥', '法律': '⚖️', '学术研究': '🎓', '项目管理': '📋',
  '支持服务': '💬', '供应链': '🚚', '游戏开发': '🎮', '付费媒体': '💳',
  '空间计算': '🥽', 'Unreal': '🎯', 'Unity': '🎯', 'Godot': '🎯',
  'Roblox': '🎯', 'Blender': '🎯', 'MCP记忆': '🧠',
}
// Watch filter/search changes — reset to page 1
watch([searchText, activeFilter, activeTags], () => { page.value = 1 })

// Paginated slice of the filtered agents
...
const groupedAgents = computed(() => {
  const groups: { category: string; icon: string; agents: any[] }[] = []
  const seen = new Set<string>()
  for (const cat of categoryOrder) {
    const items = paginatedAgents.value.filter(a => {
      const tags = agentTags(a)
      return tags.length > 0 && tags[0] === cat
    })
    if (items.length) {
      groups.push({ category: cat, icon: categoryIcons[cat] || '📌', agents: items })
      seen.add(cat)
    }
  }
  // Any remaining categories (not in categoryOrder)
  const remaining = new Map<string, any[]>()
  for (const a of paginatedAgents.value) {
    const tags = agentTags(a)
    const cat = tags.length > 0 ? tags[0] : '未分类'
    if (!seen.has(cat)) {
      if (!remaining.has(cat)) remaining.set(cat, [])
      remaining.get(cat)!.push(a)
    }
  }
  for (const [cat, items] of [...remaining.entries()].sort((a, b) => a[0].localeCompare(b[0]))) {
    groups.push({ category: cat, icon: categoryIcons[cat] || '📌', agents: items })
  }
  return groups
})
...
const isAdminRole = computed(() => ['admin','super_admin'].includes(localStorage.getItem('role') || 'user'))
...
:global(html.dark .category-header__count) {
