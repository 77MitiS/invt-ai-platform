        ...
        <div class="mc-page-header">
          ...
          <div class="header-right">
            <button class="btn-secondary" @click="triggerUpload" :disabled="uploading">
              ...
              {{ uploading ? '导入中...' : '导入Jar文件' }}
            </button>
            <button class="btn-secondary" @click="refresh" style="margin-left:8px">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            ...
            <input ref="fileInput" type="file" accept=".jar" style="display:none" @change="handleFileUpload" />
          </div>
        </div>
...
async function handleFileUpload(e: Event) {
  ...
  try {
    ...
    mcToast.success(`已导入 ${file.name}`)
    await loadPlugins()
  } catch (err: any) {
    mcToast.error(err.message || '导入失败')
  } finally {
