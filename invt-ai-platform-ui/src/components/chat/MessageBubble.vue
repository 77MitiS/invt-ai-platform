  ...
  >
    ...
    <div class="msg-avatar" :class="`${role}-avatar`">
      <slot name="avatar">
        ...
        >
          <img src="/logo/ai-avatar.png" alt="" class="avatar-logo" />
        </GoalAvatarRing>
      ...
      <!-- 消息操作栏：始终占位，hover 时显示 -->
        ...
        >
          ...
          <button
            v-if="role === 'assistant' && !isGenerating"
            class="action-btn"
            :class="{ 'feedback-active': feedbackState === 'like' }"
            type="button"
            :title="feedbackState === 'like' ? $t('chat.feedbackSubmitted') : $t('chat.like')"
            @click="handleFeedback('like')"
          >
            <svg width="15" height="15" viewBox="0 0 24 24" :fill="feedbackState === 'like' ? 'currentColor' : 'none'" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
          ...
          <button
            v-if="role === 'assistant' && !isGenerating"
            class="action-btn"
            :class="{ 'feedback-active': feedbackState === 'dislike' }"
            type="button"
            :title="feedbackState === 'dislike' ? $t('chat.feedbackSubmitted') : $t('chat.dislike')"
            @click="handleFeedback('dislike')"
          >
            <svg width="15" height="15" viewBox="0 0 24 24" :fill="feedbackState === 'dislike' ? 'currentColor' : 'none'" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
...
import { http, conversationApi } from '@/api'
...
// --- 点赞 / 点踩反馈 ---
const feedbackState = ref<'idle' | 'like' | 'dislike'>('idle')
...
async function handleFeedback(rating: 'like' | 'dislike') {
  ...
  if (feedbackState.value === rating) {
    feedbackState.value = 'idle'
    return
  }
  try {
    await conversationApi.submitFeedback(cid, Number(mid), rating)
...
.action-btn.feedback-active {
  color: var(--mc-primary, #E31837);
}
.tts-loading-icon {
  animation: spin 1s linear infinite;
}
