export default {
  ...
  chat: {
    ...
    subtitle: '记忆 · 知识库 · 技能 · 自动化 —— 你的个人 AI 智能体',
    // 排队相关
  ...
  settings: {
    ...
    aboutDesc: '版本与系统信息',
    about: {
      heroDesc: '一个企业级的AI智能体平台，让模型真正进入上下文、记忆、执行、知识和交付的连续流程。',
      foundationTitle: '技术栈',
      pillars: {
    ...
    model: {
      ...
      claudeCodeOauthRevokeHint: '请在 Claude Code 客户端中退出登录。Invt 不会修改 Claude Code 的本地凭据。',
  ...
  acp: {
    ...
    env: {
      ...
      hints: {
        ...
        dashscope: '阿里云 DashScope key — 跟 Invt .env 里的 DASHSCOPE_API_KEY 同一个值',
  ...
  plugins: {
    ...
    noDescription: '暂无描述',
    emptyTitle: '暂无已安装插件',
    emptyHint: '将插件 JAR 文件放入 ~/.invt/plugins/ 目录后重启服务',
    loading: '加载插件中...',
    loadFailed: '加载插件失败',
    enabled: '{name} 已启用',
    disabled: '{name} 已禁用',
  ...
  wiki: {
    ...
    library: {
      ...
      scopePublicHint: '工作区共享',
      scopePrivateHint: '仅自己可见',
      empty: '还没有知识库',
  ...
  login: {
    ...
    subtitle: '你的企业级 AI 智能体平台',
    heroTitle: '让 AI 不止会回答，而是真正进入持续工作的系统。',
    heroDesc: 'INVT AI Platform 的重点不是把更多能力堆进聊天框，而是把上下文、知识、执行和记忆收拢成一个可靠的工作面。',
  ...
  channels: {
    ...
    webchatHint: 'Web / API 接入既可把 Invt 聊天挂件嵌入外部网站，也可作为纯后端 SSE 接口直接调用。配置 API Key 后，前端可引入 WebChat SDK；后端可直接 POST /api/v1/channels/webchat/stream，头带 X-MC-Key、体传 {message, visitorId, 可选 sessionId/agentId}。',
