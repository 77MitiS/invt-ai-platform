...
http.interceptors.request.use((config) => {
  ...
  // in sync.
  const locale = localStorage.getItem('invt_locale')
  if (locale) {
...
export const conversationApi = {
  ...
  submitFeedback: (conversationId: string, messageId: number, rating: 'like' | 'dislike') =>
    http.put(`/conversations/${conversationId}/messages/${messageId}/feedback`, { rating }),
}
...
export const skillApi = {
  ...
  /** Export skill config as JSON (download) */
  exportConfig: (id: string | number) => http.get(`/skills/${id}/export`),
  /** Generate a share link/token for a skill */
  share: (id: string | number) => http.post(`/skills/${id}/share`),
}
...
export const modelApi = {
  listProviders: () => http.get('/models/chat-providers'),
...
export const oauthApi = {
  ...
  // Device Authorization Grant — used when Invt runs on a remote host so the
...
export const pluginApi = {
  ...
  upload: (file: File) => {
    const form = new FormData()
    form.append('file', file)
    return http.post('/plugins/upload', form, { headers: { 'Content-Type': 'multipart/form-data' } })
  },
}
...
export const enterpriseApi = {
  getContracts: () => http.get('/enterprise/contracts'),
  saveContract: (data: any) => http.post('/enterprise/contracts', data),
  deleteContract: (id: string) => http.delete(`/enterprise/contracts/${id}`),
  getAccounts: () => http.get('/enterprise/accounts'),
  saveAccount: (data: any) => http.post('/enterprise/accounts', data),
  deleteAccount: (id: string) => http.delete(`/enterprise/accounts/${id}`),
}
